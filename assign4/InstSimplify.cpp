...
static Value *simplifyUnOp (unsigned /*Opcode*/, Value * /*Op*/, const SimplifyQuery & /*Q*/,
                            unsigned MaxRecurse = 1000);
static Value *simplifyFPUnOp (unsigned /*Opcode*/, Value * /*Op*/, const FastMathFlags & /*FMF*/,
                              const SimplifyQuery & /*Q*/, unsigned MaxRecurse = 1000);
static Value *SimplifyBinOp (unsigned /*Opcode*/, Value * /*LHS*/, Value * /*RHS*/, const SimplifyQuery & /*Q*/,
                             unsigned MaxRecurse = 1000);
static Value *SimplifyBinOp (unsigned /*Opcode*/, Value * /*LHS*/, Value * /*RHS*/, const FastMathFlags & /*FMF*/,
                             const SimplifyQuery & /*Q*/, unsigned MaxRecurse = 1000);
static Value *SimplifyCmpInst (unsigned /*Predicate*/, Value * /*LHS*/, Value * /*RHS*/, const SimplifyQuery & /*Q*/,
                               unsigned MaxRecurse = 1000);
static Value *SimplifyICmpInst (unsigned Predicate, Value *LHS, Value *RHS, const SimplifyQuery &Q,
                                unsigned MaxRecurse = 1000);
...
static Value *SimplifyXorInst (Value * /*Op0*/, Value * /*Op1*/, const SimplifyQuery & /*Q*/,
                               unsigned MaxRecurse = 1000);
static Value *SimplifyCastInst (unsigned /*CastOpc*/, Value * /*Op*/, Type * /*Ty*/, const SimplifyQuery & /*Q*/,
                                unsigned MaxRecurse = 1000);
static Value *SimplifyGEPInst (Type * /*SrcTy*/, Value * /*Ptr*/, ArrayRef<Value *> /*Indices*/, bool /*InBounds*/,
                               const SimplifyQuery & /*Q*/, unsigned MaxRecurse = 1000);
static Value *SimplifySelectInst (Value * /*Cond*/, Value * /*TrueVal*/, Value * /*FalseVal*/,
                                  const SimplifyQuery & /*Q*/, unsigned MaxRecurse = 1000);

static Value *
foldSelectWithBinaryOp (Value *Cond, Value *TrueVal, Value *FalseVal)
{
  ...
  Value *X;
  Value *Y;
  if (!match (Cond, m_c_BinOp (m_c_ICmp (Pred1, m_Specific (TrueVal), m_Specific (FalseVal)),
...
static bool
isSameCompare (Value *V, CmpInst::Predicate Pred, Value *LHS, Value *RHS)
{
  ...
  Value             *CLHS  = Cmp->getOperand (0);
  Value             *CRHS  = Cmp->getOperand (1);
...
static Value *
simplifyCmpSelCase (CmpInst::Predicate Pred, Value *LHS, Value *RHS, Value *Cond, const SimplifyQuery &Q,
...
static Value *
simplifyCmpSelTrueCase (CmpInst::Predicate Pred, Value *LHS, Value *RHS, Value *Cond, const SimplifyQuery &Q,
                        unsigned MaxRecurse)
{
...
static Value *
simplifyCmpSelFalseCase (CmpInst::Predicate Pred, Value *LHS, Value *RHS, Value *Cond, const SimplifyQuery &Q,
                         unsigned MaxRecurse)
{
...
static Value *
handleOtherCmpSelSimplifications (Value *TCmp, Value *FCmp, Value *Cond, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  if (match (FCmp, m_One ()) && match (TCmp, m_Zero ()))
    {
      if (Value *V = SimplifyXorInst (Cond, Constant::getAllOnesValue (Cond->getType ()), Q, MaxRecurse))
        {
          return V;
        }
    }
...
valueDominatesPHI (Value *V, PHINode *P, const DominatorTree *DT)
{
  ...
  if ((I->getParent () == nullptr) || (P->getParent () == nullptr) || (I->getFunction () == nullptr))
    {
      return false;
    }
  ...
  if (I->getParent ()->isEntryBlock () && !isa<InvokeInst> (I) && !isa<CallBrInst> (I))
    {
      return true;
    }
...
static Value *
expandBinOp (Instruction::BinaryOps Opcode, Value *V, Value *OtherOp, Instruction::BinaryOps OpcodeToExpand,
             const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  Value *B0 = B->getOperand (0);
  Value *B1 = B->getOperand (1);
  Value *L  = SimplifyBinOp (Opcode, B0, OtherOp, Q.getWithoutUndef (), MaxRecurse);
  if (L == nullptr)
    {
      return nullptr;
    }
  Value *R = SimplifyBinOp (Opcode, B1, OtherOp, Q.getWithoutUndef (), MaxRecurse);
  if (R == nullptr)
    {
      return nullptr;
    }
  ...
  if ((L == B0 && R == B1) || (Instruction::isCommutative (OpcodeToExpand) && L == B1 && R == B0))
    {
      ++NumExpand;
      return B;
    }
...
static Value *
expandCommutativeBinOp (Instruction::BinaryOps Opcode, Value *L, Value *R, Instruction::BinaryOps OpcodeToExpand,
                        const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
static Value *
SimplifyAssociativeBinOp (Instruction::BinaryOps Opcode, Value *LHS, Value *RHS, const SimplifyQuery &Q,
                          unsigned MaxRecurse)
{
  ...
  // Transform: "(A op B) op C" ==> "A op (B op C)" if it simplifies
  // completely.
  if ((Op0 != nullptr) && Op0->getOpcode () == Opcode)
    {
      ...
      if (Value *V = SimplifyBinOp (Opcode, B, C, Q, MaxRecurse))
        {
          // It does!  Return "A op V" if it simplifies or is already
          // available. If V equals B then "A op V" is just the LHS.
          if (V == B)
            {
              return LHS;
            }
  ...
  if ((Op1 != nullptr) && Op1->getOpcode () == Opcode)
    {
      ...
      if (Value *V = SimplifyBinOp (Opcode, A, B, Q, MaxRecurse))
        {
          // It does!  Return "V op C" if it simplifies or is already
          // available. If V equals B then "V op C" is just the RHS.
          if (V == B)
            {
              return RHS;
            }
  ...
  // Transform: "(A op B) op C" ==> "(C op A) op B" if it simplifies
  // completely.
  if ((Op0 != nullptr) && Op0->getOpcode () == Opcode)
    {
      Value *A = Op0->getOperand (0);
      ...
      if (Value *V = SimplifyBinOp (Opcode, C, A, Q, MaxRecurse))
        {
          // It does!  Return "V op B" if it simplifies or is already
          // available. If V equals A then "V op B" is just the LHS.
          if (V == A)
            {
              return LHS;
            }
  ...
  // Transform: "A op (B op C)" ==> "B op (C op A)" if it simplifies
  // completely.
  if ((Op1 != nullptr) && Op1->getOpcode () == Opcode)
    {
      Value *A = LHS;
      ...
      if (Value *V = SimplifyBinOp (Opcode, C, A, Q, MaxRecurse))
        {
          // It does!  Return "B op V" if it simplifies or is already
          // available. If V equals C then "B op V" is just the RHS.
          if (V == C)
            {
              return RHS;
            }
...
static Value *ThreadBinOpOverSelect (Instruction::BinaryOps Opcode, Value *LHS, Value *RHS, const SimplifyQuery &Q,
                                     unsigned MaxRecurse);
...
static Value *
ThreadCmpOverSelect (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  // Recursion is always used, so bail out at once if we already hit the limit.
  if ((MaxRecurse--) == 0u)
    {
...
/// In the case of a binary operation with an operand that is a PHI
/// instruction, try to simplify the binop by seeing whether evaluating it on
/// the incoming phi values yields the same result for every value. If so
/// returns the common value, otherwise returns null.
static Value *
ThreadBinOpOverPHI (Instruction::BinaryOps Opcode, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  // Recursion is always used, so bail out at once if we already hit the limit.
  if ((MaxRecurse--) == 0u)
    {
  ...
  for (Value *Incoming : PI->incoming_values ())
    {
      // If the incoming value is the phi node itself, it can safely be
      // skipped.
      if (Incoming == PI)
        {
          continue;
      ...
      // If the operation failed to simplify, or simplified to a different
      // value to previously, then give up.
      if ((V == nullptr) || ((CommonValue != nullptr) && V != CommonValue))
        {
          return nullptr;
        }
      CommonValue = V;
...
static Value *
ThreadCmpOverPHI (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  // Recursion is always used, so bail out at once if we already hit the limit.
  if ((MaxRecurse--) == 0u)
    {
  ...
  for (unsigned u = 0, e = PI->getNumIncomingValues (); u < e; ++u)
    {
      ...
      // This is important because that is where incoming is actually
      // "evaluated" even though it is used later somewhere else.
...
static Constant *
foldOrCommuteConstant (Instruction::BinaryOps Opcode, Value *&Op0, Value *&Op1, const SimplifyQuery &Q)
{
  if (auto *CLHS = dyn_cast<Constant> (Op0))
    {
      ...
      // Canonicalize the constant to the RHS if this is a commutative
      // operation.
      if (Instruction::isCommutative (Opcode))
        {
          std::swap (Op0, Op1);
...
static Value *
SimplifyAddInst (Value *Op0, Value *Op1, bool IsNSW, bool IsNUW, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  // Eg: X + -X -> 0
  Value *Y = nullptr;
  if (match (Op1, m_Sub (m_Value (Y), m_Specific (Op0))) || match (Op0, m_Sub (m_Value (Y), m_Specific (Op1))))
    {
      return Y;
    }
  ...
  if ((IsNSW || IsNUW) && match (Op1, m_SignMask ()) && match (Op0, m_Xor (m_Value (Y), m_SignMask ())))
    {
      return Y;
    }
  ...
  if (Value *V = SimplifyAssociativeBinOp (Instruction::Add, Op0, Op1, Q, MaxRecurse))
    {
      return V;
    }
...
Value *
SimplifyAddInst (Value *Op0, Value *Op1, bool IsNSW, bool IsNUW, const SimplifyQuery &Query)
{
...
/// folding.
static Constant *
stripAndComputeConstantOffsets (const DataLayout &DL, Value *&V, bool AllowNonInbounds = false)
{
...
static Constant *
computePointerDifference (const DataLayout &DL, Value *LHS, Value *RHS)
{
...
/// If not, this returns null.
static Value *
SimplifySubInst (Value *Op0, Value *Op1, bool isNSW, bool isNUW, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  if (match (Op0, m_Zero ()))
    {
      ...
      if (Known.Zero.isMaxSignedValue ())
        {
          // Op1 is either 0 or the minimum signed value. If the sub is NSW,
          // then Op1 must be 0 because negating the minimum signed value is
          // undefined.
          if (isNSW)
            {
              return Constant::getNullValue (Op0->getType ());
  ...
  Value *Y = nullptr;
  Value *Z = Op1;
  if ((MaxRecurse != 0u) && match (Op0, m_Add (m_Value (X), m_Value (Y))))
    { // (X + Y) - Z
  ...
  // For example, X - (X + 1) -> -1
  X = Op0;
  if ((MaxRecurse != 0u) && match (Op1, m_Add (m_Value (Y), m_Value (Z))))
    { // X - (Y + Z)
  ...
  // For example, X - (X - Y) -> Y.
  Z = Op0;
  if ((MaxRecurse != 0u) && match (Op1, m_Sub (m_Value (X), m_Value (Y))))
    { // Z - (X - Y)
  ...
  if ((MaxRecurse != 0u) && match (Op0, m_Trunc (m_Value (X))) && match (Op1, m_Trunc (m_Value (Y))))
    {
...
/// If not, this returns null.
static Value *
SimplifyMulInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  if (Q.IIQ.UseInstrInfo
      && (match (Op0, m_Exact (m_IDiv (m_Value (X), m_Specific (Op1)))) || // (X / Y) * Y
          match (Op1, m_Exact (m_IDiv (m_Value (X), m_Specific (Op0))))))
    { // Y * (X / Y)
      return X;
    }
  ...
  // Try some generic simplifications for associative operations.
  if (Value *V = SimplifyAssociativeBinOp (Instruction::Mul, Op0, Op1, Q, MaxRecurse))
    {
      return V;
    }

  // Mul distributes over Add. Try some generic simplifications based on this.
  if (Value *V = expandCommutativeBinOp (Instruction::Mul, Op0, Op1, Instruction::Add, Q, MaxRecurse))
    {
      return V;
    }
...
static Value *
simplifyDivRem (Instruction::BinaryOps Opcode, Value *Op0, Value *Op1, const SimplifyQuery &Q)
{
      ...
      for (unsigned i = 0; i != NumElts; ++i)
        {
          ...
          if ((Elt != nullptr) && (Elt->isNullValue () || Q.isUndefValue (Elt)))
            {
              return PoisonValue::get (Ty);
            }
        }
  ...
  // Similarly, if we're zero-extending a boolean divisor, then assume it's
  // a 1.
  Value *X;
  ...
  if (match (Op0, m_c_Mul (m_Value (X), m_Specific (Op1))))
    {
      ...
      // X == A / Y for some A.
      if ((IsSigned && Q.IIQ.hasNoSignedWrap (Mul)) || (!IsSigned && Q.IIQ.hasNoUnsignedWrap (Mul))
...
/// This is a helper for div/rem simplification where we return some other
/// value when we can prove a relationship between the operands.
static bool
isICmpTrue (ICmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
static bool
isDivZero (Value *X, Value *Y, const SimplifyQuery &Q, unsigned MaxRecurse, bool IsSigned)
{
  // Recursion is always used, so bail out at once if we already hit the limit.
  if ((MaxRecurse--) == 0u)
    {
  ...
  if (IsSigned)
    {
      // |X| / |Y| --> 0
      //
      // We require that 1 operand is a simple constant. That could be extended
      // to 2 variables if we computed the sign bit for each.
      //
      ...
      if (match (Y, m_APInt (C)))
        {
          // Special-case: we can't take the abs() of a minimum signed value.
          // If that's the divisor, then all we have to do is prove that the
          // dividend is also not the minimum signed value.
  ...
  const APInt *C;
  if (match (Y, m_APInt (C)) && computeKnownBits (X, Q.DL, 0, Q.AC, Q.CxtI, Q.DT).getMaxValue ().ult (*C))
    {
      return true;
    }
...
/// otherwise returns null.
static Value *
ThreadBinOpOverSelect (Instruction::BinaryOps Opcode, Value *LHS, Value *RHS, const SimplifyQuery &Q,
                       unsigned MaxRecurse)
{
  // Recursion is always used, so bail out at once if we already hit the limit.
  if ((MaxRecurse--) == 0u)
  ...
  if (((FV != nullptr) && (TV == nullptr)) || ((TV != nullptr) && (FV == nullptr)))
    {
      // Check that the simplified value has the form "X op Y" where "op" is
      // the same as the original operation.
      Instruction *Simplified = dyn_cast<Instruction> ((FV != nullptr) ? FV : TV);
      if ((Simplified != nullptr) && Simplified->getOpcode () == unsigned (Opcode))
        {
          // The value that didn't simplify is "UnsimplifiedLHS op
          // UnsimplifiedRHS". We already know that "op" is the same as for the
          // simplified value.  See if the operands match too.  If so, return
          // the simplified value.
          Value *UnsimplifiedBranch = (FV != nullptr) ? SI->getTrueValue () : SI->getFalseValue ();
          ...
          if (Simplified->isCommutative () && Simplified->getOperand (1) == UnsimplifiedLHS
...
static Value *
simplifyDiv (Instruction::BinaryOps Opcode, Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  ConstantInt *C1;
  ConstantInt *C2;
  if (!IsSigned && match (Op0, m_UDiv (m_Value (), m_ConstantInt (C1))) && match (Op1, m_ConstantInt (C2)))
    {
      bool Overflow;
...
static Value *
simplifyRem (Instruction::BinaryOps Opcode, Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  if (Constant *C = foldOrCommuteConstant (Opcode, Op0, Op1, Q))
    {
      return C;
  ...
  // (X % Y) % Y -> X % Y
  if ((Opcode == Instruction::SRem && match (Op0, m_SRem (m_Value (), m_Specific (Op1))))
      || (Opcode == Instruction::URem && match (Op0, m_URem (m_Value (), m_Specific (Op1)))))
    {
      return Op0;
    }
  ...
  // (X << Y) % X -> 0
  if (Q.IIQ.UseInstrInfo
      && ((Opcode == Instruction::SRem && match (Op0, m_NSWShl (m_Specific (Op1), m_Value ())))
          || (Opcode == Instruction::URem && match (Op0, m_NUWShl (m_Specific (Op1), m_Value ())))))
    {
      return Constant::getNullValue (Op0->getType ());
    }
...
/// If not, this returns null.
static Value *
SimplifySDivInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
/// If not, this returns null.
static Value *
SimplifyUDivInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
/// If not, this returns null.
static Value *
SimplifySRemInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
/// If not, this returns null.
static Value *
SimplifyURemInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
static bool
isPoisonShift (Value *Amount, const SimplifyQuery &Q)
{
  ...
  if (isa<ConstantVector> (C) || isa<ConstantDataVector> (C))
    {
      for (unsigned I = 0, E = cast<FixedVectorType> (C->getType ())->getNumElements (); I != E; ++I)
        {
...
static Value *SimplifyShift (Instruction::BinaryOps Opcode, Value *Op0, Value *Op1, bool IsNSW, const SimplifyQuery &Q,
                             unsigned MaxRecurse);

/// Given operands for an Shl, LShr or AShr, see if we can
/// fold the result.  If not, this returns null.
static Value *
SimplifyRightShift (Instruction::BinaryOps Opcode, Value *Op0, Value *Op1, bool isExact, const SimplifyQuery &Q,
                    unsigned MaxRecurse)
{
...
/// If not, this returns null.
static Value *
SimplifyShlInst (Value *Op0, Value *Op1, bool isNSW, bool isNUW, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  if (Value *V = SimplifyShift (Instruction::Shl, Op0, Op1, isNSW, Q, MaxRecurse))
    {
      return V;
    }
  ...
  // (X >> A) << A -> X
  Value *X;
  if (Q.IIQ.UseInstrInfo && match (Op0, m_Exact (m_Shr (m_Value (X), m_Specific (Op1)))))
    {
      return X;
    }
...
/// If not, this returns null.
static Value *
SimplifyLShrInst (Value *Op0, Value *Op1, bool isExact, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  if (Value *V = SimplifyRightShift (Instruction::LShr, Op0, Op1, isExact, Q, MaxRecurse))
    {
      return V;
    }
  ...
  const APInt *ShRAmt;
  const APInt *ShLAmt;
  if (match (Op1, m_APInt (ShRAmt)) && match (Op0, m_c_Or (m_NUWShl (m_Value (X), m_APInt (ShLAmt)), m_Value (Y)))
      && *ShRAmt == *ShLAmt)
    {
...
/// If not, this returns null.
static Value *
SimplifyAShrInst (Value *Op0, Value *Op1, bool isExact, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  if (match (Op0, m_AllOnes ()) || match (Op0, m_Shl (m_AllOnes (), m_Specific (Op1))))
    {
      return Constant::getAllOnesValue (Op0->getType ());
    }
...
static Value *
simplifyUnsignedRangeCheck (ICmpInst *ZeroICmp, ICmpInst *UnsignedICmp, bool IsAnd, const SimplifyQuery &Q)
{
  Value *X;
  Value *Y;

  ICmpInst::Predicate EqPred;
  if (!match (ZeroICmp, m_ICmp (EqPred, m_Value (Y), m_Zero ())) || !ICmpInst::isEquality (EqPred))
    {
      return nullptr;
    }
  ...
  if (match (Y, m_Sub (m_Value (A), m_Value (B))))
    {
      if (match (UnsignedICmp, m_c_ICmp (UnsignedPred, m_Specific (A), m_Specific (B)))
          && ICmpInst::isUnsigned (UnsignedPred))
        {
          // A >=/<= B || (A - B) != 0  <-->  true
          if ((UnsignedPred == ICmpInst::ICMP_UGE || UnsignedPred == ICmpInst::ICMP_ULE) && EqPred == ICmpInst::ICMP_NE
              && !IsAnd)
            {
              return ConstantInt::getTrue (UnsignedICmp->getType ());
            }
          // A </> B && (A - B) == 0  <-->  false
          if ((UnsignedPred == ICmpInst::ICMP_ULT || UnsignedPred == ICmpInst::ICMP_UGT) && EqPred == ICmpInst::ICMP_EQ
              && IsAnd)
            {
          ...
          // A </> B && (A - B) != 0  <-->  A </> B
          // A </> B || (A - B) != 0  <-->  (A - B) != 0
          if (EqPred == ICmpInst::ICMP_NE && (UnsignedPred == ICmpInst::ICMP_ULT || UnsignedPred == ICmpInst::ICMP_UGT))
            {
          ...
          // A <=/>= B && (A - B) == 0  <-->  (A - B) == 0
          // A <=/>= B || (A - B) == 0  <-->  A <=/>= B
          if (EqPred == ICmpInst::ICMP_EQ && (UnsignedPred == ICmpInst::ICMP_ULE || UnsignedPred == ICmpInst::ICMP_UGE))
            {
      ...
      //   Y >= A && Y != 0  --> Y >= A  iff B != 0
      //   Y <  A || Y == 0  --> Y <  A  iff B != 0
      if (match (UnsignedICmp, m_c_ICmp (UnsignedPred, m_Specific (Y), m_Specific (A))))
        {
          if (UnsignedPred == ICmpInst::ICMP_UGE && IsAnd && EqPred == ICmpInst::ICMP_NE
  ...
  if (match (UnsignedICmp, m_ICmp (UnsignedPred, m_Value (X), m_Specific (Y))) && ICmpInst::isUnsigned (UnsignedPred))
    {
      ;
    }
  else if (match (UnsignedICmp, m_ICmp (UnsignedPred, m_Specific (Y), m_Value (X)))
           && ICmpInst::isUnsigned (UnsignedPred))
    {
  ...
  // foldAndOrOfICmpsWithConstEq(). If we are looking to trim optimizer
  // overlap, these are candidates for removal.
  ...
  // X < Y && Y == 0  -->  false
  if (UnsignedPred == ICmpInst::ICMP_ULT && EqPred == ICmpInst::ICMP_EQ && IsAnd)
    {
      return getFalse (UnsignedICmp->getType ());
    }

  // X >= Y || Y != 0  -->  true
  if (UnsignedPred == ICmpInst::ICMP_UGE && EqPred == ICmpInst::ICMP_NE && !IsAnd)
    {
...
simplifyAndOfICmpsWithSameOperands (ICmpInst *Op0, ICmpInst *Op1)
{
  ...
  // Check for any combination of predicates that are guaranteed to be
  // disjoint.
...
/// the other.
static Value *
simplifyAndOrOfICmpsWithConstants (ICmpInst *Cmp0, ICmpInst *Cmp1, bool IsAnd)
{
  ...
  const APInt *C0;
  const APInt *C1;
  if (!match (Cmp0->getOperand (1), m_APInt (C0)) || !match (Cmp1->getOperand (1), m_APInt (C1)))
    {
      return nullptr;
    }
...
static Value *
simplifyAndOrOfICmpsWithZero (ICmpInst *Cmp0, ICmpInst *Cmp1, bool IsAnd)
{
  ICmpInst::Predicate P0 = Cmp0->getPredicate ();
  ICmpInst::Predicate P1 = Cmp1->getPredicate ();
  if (!match (Cmp0->getOperand (1), m_Zero ()) || !match (Cmp1->getOperand (1), m_Zero ()) || P0 != P1)
    {
      return nullptr;
    }
  ...
  // that compare implies the other, so we eliminate the other. Optionally,
  // look through a pointer-to-int cast to match a null check of a pointer
  // type.
  ...
  // (X != 0) && (([ptrtoint] X & ?) != 0) --> ([ptrtoint] X & ?) != 0
  // (X != 0) && ((? & [ptrtoint] X) != 0) --> (? & [ptrtoint] X) != 0
  if (match (Y, m_c_And (m_Specific (X), m_Value ())) || match (Y, m_c_And (m_PtrToInt (m_Specific (X)), m_Value ())))
    {
      return Cmp1;
    }
  ...
  // (([ptrtoint] Y & ?) != 0) && (Y != 0) --> ([ptrtoint] Y & ?) != 0
  // ((? & [ptrtoint] Y) != 0) && (Y != 0) --> (? & [ptrtoint] Y) != 0
  if (match (X, m_c_And (m_Specific (Y), m_Value ())) || match (X, m_c_And (m_PtrToInt (m_Specific (Y)), m_Value ())))
    {
      return Cmp0;
    }
...
static Value *
simplifyAndOfICmpsWithAdd (ICmpInst *Op0, ICmpInst *Op1, const InstrInfoQuery &IIQ)
{
...
static Value *
simplifyAndOrOfICmpsWithLimitConst (ICmpInst *Cmp0, ICmpInst *Cmp1, bool IsAnd)
{
...
static Value *
simplifyAndOfICmps (ICmpInst *Op0, ICmpInst *Op1, const SimplifyQuery &Q)
{
...
static Value *
simplifyOrOfICmpsWithAdd (ICmpInst *Op0, ICmpInst *Op1, const InstrInfoQuery &IIQ)
{
  // (icmp (add V, C0), C1) | (icmp V, C0)
  ICmpInst::Predicate Pred0;
  ICmpInst::Predicate Pred1;
...
static Value *
simplifyOrOfICmps (ICmpInst *Op0, ICmpInst *Op1, const SimplifyQuery &Q)
{
...
/// If not, this returns null.
static Value *
SimplifyShift (Instruction::BinaryOps Opcode, Value *Op0, Value *Op1, bool IsNSW, const SimplifyQuery &Q,
               unsigned MaxRecurse)
{
  if (Constant *C = foldOrCommuteConstant (Opcode, Op0, Op1, Q))
    {
  ...
  // would be poison.
  Value *X;
  if (match (Op1, m_Zero ()) || (match (Op1, m_SExt (m_Value (X))) && X->getType ()->isIntOrIntVectorTy (1)))
    {
      return Op0;
    }
...
static Value *
simplifyAndOrOfFCmps (const TargetLibraryInfo *TLI, FCmpInst *LHS, FCmpInst *RHS, bool IsAnd)
{
  Value *LHS0 = LHS->getOperand (0);
  Value *LHS1 = LHS->getOperand (1);
...
static Value *
simplifyAndOrOfCmps (const SimplifyQuery &Q, Value *Op0, Value *Op1, bool IsAnd)
{
  // Look through casts of the 'and' operands to find compares.
  auto *Cast0 = dyn_cast<CastInst> (Op0);
  auto *Cast1 = dyn_cast<CastInst> (Op1);
  if ((Cast0 != nullptr) && (Cast1 != nullptr) && Cast0->getOpcode () == Cast1->getOpcode ()
  ...
  if ((ICmp0 != nullptr) && (ICmp1 != nullptr))
    {
      V = IsAnd ? simplifyAndOfICmps (ICmp0, ICmp1, Q) : simplifyOrOfICmps (ICmp0, ICmp1, Q);
    }
...
static Value *
simplifyLogicOfAddSub (Value *Op0, Value *Op1, Instruction::BinaryOps Opcode)
{
  ...
  Constant *C1;
  Constant *C2;
  if ((match (Op0, m_Add (m_Value (X), m_Constant (C1))) && match (Op1, m_Sub (m_Constant (C2), m_Specific (X))))
      || (match (Op1, m_Add (m_Value (X), m_Constant (C1))) && match (Op0, m_Sub (m_Constant (C2), m_Specific (X)))))
    {
      if (ConstantExpr::getNot (C1) == C2)
        {
          ...
          // (X + C) ^ (~C - X) --> (X + C) ^ ~(X + C) --> -1
          Type *Ty = Op0->getType ();
          return Opcode == Instruction::And ? ConstantInt::getNullValue (Ty) : ConstantInt::getAllOnesValue (Ty);
        }
    }
  return nullptr;
}
...
/// If not, this returns null.
static Value *
SimplifyAndInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  Value *X;
  Value *Y;
  if (match (Op0, m_c_Or (m_Value (X), m_Not (m_Value (Y)))) && match (Op1, m_c_Or (m_Deferred (X), m_Deferred (Y))))
    {
      return X;
    }
  if (match (Op1, m_c_Or (m_Value (X), m_Not (m_Value (Y)))) && match (Op0, m_c_Or (m_Deferred (X), m_Deferred (Y))))
    {
      return X;
    }
  ...
  if (match (Op1, m_APInt (Mask)))
    {
      ...
      if (match (Op0, m_Shl (m_Value (X), m_APInt (ShAmt))) && (~(*Mask)).lshr (*ShAmt).isZero ())
        {
          return Op0;
        }
      ...
      if (match (Op0, m_LShr (m_Value (X), m_APInt (ShAmt))) && (~(*Mask)).shl (*ShAmt).isZero ())
        {
          return Op0;
        }
    }
  ...
  if (match (Op0, m_Neg (m_Specific (Op1))) || match (Op1, m_Neg (m_Specific (Op0))))
    {
      if (isKnownToBeAPowerOfTwo (Op0, Q.DL, /*OrZero*/ true, 0, Q.AC, Q.CxtI, Q.DT))
        {
          return Op0;
        }
      if (isKnownToBeAPowerOfTwo (Op1, Q.DL, /*OrZero*/ true, 0, Q.AC, Q.CxtI, Q.DT))
        {
          return Op1;
        }
    }
  ...
  // Try some generic simplifications for associative operations.
  if (Value *V = SimplifyAssociativeBinOp (Instruction::And, Op0, Op1, Q, MaxRecurse))
    {
      return V;
    }
      ...
      if (Op0->getType ()->isIntOrIntVectorTy (1))
        {
          ...
          if (match (Op0, m_Select (m_Specific (Op1), m_Value (), m_Zero ())))
            return Op0;
        }
      // If the operation is with the result of a select instruction, check
      // whether operating on either branch of the select always yields the
      // same value.
  ...
  Value *XShifted;
  if (match (Op1, m_APInt (Mask))
      && match (Op0, m_c_Or (m_CombineAnd (m_NUWShl (m_Value (X), m_APInt (ShAmt)), m_Value (XShifted)), m_Value (Y))))
    {
  ...
  // ((X | Y) ^ Y ) & ((X | Y) ^ X) --> 0
  BinaryOperator *Or;
  if (match (Op0, m_c_Xor (m_Value (X), m_CombineAnd (m_BinOp (Or), m_c_Or (m_Deferred (X), m_Value (Y)))))
...
static Value *
simplifyOrLogic (Value *X, Value *Y)
{
  ...
  // (A ^ B) | (A | B) --> A | B
  // (A ^ B) | (B | A) --> B | A
  if (match (X, m_Xor (m_Value (A), m_Value (B))) && match (Y, m_c_Or (m_Specific (A), m_Specific (B))))
    {
      return Y;
    }
  ...
  // ~(A ^ B) | (A | B) --> -1
  // ~(A ^ B) | (B | A) --> -1
  if (match (X, m_Not (m_Xor (m_Value (A), m_Value (B)))) && match (Y, m_c_Or (m_Specific (A), m_Specific (B))))
    {
      return ConstantInt::getAllOnesValue (Ty);
    }
  ...
  // (A & ~B) | (B ^ A) --> B ^ A
  // (~B & A) | (B ^ A) --> B ^ A
  if (match (X, m_c_And (m_Value (A), m_Not (m_Value (B)))) && match (Y, m_c_Xor (m_Specific (A), m_Specific (B))))
    {
      return Y;
    }
  ...
  // (~A ^ B) | (B & A) --> ~A ^ B
  // (B ^ ~A) | (B & A) --> B ^ ~A
  if (match (X, m_c_Xor (m_Not (m_Value (A)), m_Value (B))) && match (Y, m_c_And (m_Specific (A), m_Specific (B))))
    {
      return X;
    }
  ...
  // (B | ~A) | (A ^ B) --> -1
  // (B | ~A) | (B ^ A) --> -1
  if (match (X, m_c_Or (m_Not (m_Value (A)), m_Value (B))) && match (Y, m_c_Xor (m_Specific (A), m_Specific (B))))
    {
      return ConstantInt::getAllOnesValue (Ty);
    }
  ...
  // (B & ~A) | ~(B | A) --> ~A
  Value *NotA;
  if (match (X, m_c_And (m_CombineAnd (m_Value (NotA), m_NotForbidUndef (m_Value (A))), m_Value (B)))
  ...
  // ~(A ^ B) | (B & A) --> ~(A ^ B)
  Value *NotAB;
  if (match (X, m_CombineAnd (m_NotForbidUndef (m_Xor (m_Value (A), m_Value (B))), m_Value (NotAB)))
  ...
  // ~(A & B) | (A ^ B) --> ~(A & B)
  // ~(A & B) | (B ^ A) --> ~(A & B)
  if (match (X, m_CombineAnd (m_NotForbidUndef (m_And (m_Value (A), m_Value (B))), m_Value (NotAB)))
...
/// If not, this returns null.
static Value *
SimplifyOrInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  Value *X;
  Value *Y;
  if ((match (Op0, m_Shl (m_AllOnes (), m_Value (X))) && match (Op1, m_LShr (m_AllOnes (), m_Value (Y))))
      || (match (Op1, m_Shl (m_AllOnes (), m_Value (X))) && match (Op0, m_LShr (m_AllOnes (), m_Value (Y)))))
    {
      const APInt *C;
      if ((match (X, m_Sub (m_APInt (C), m_Specific (Y))) || match (Y, m_Sub (m_APInt (C), m_Specific (X))))
  ...
  // Try some generic simplifications for associative operations.
  if (Value *V = SimplifyAssociativeBinOp (Instruction::Or, Op0, Op1, Q, MaxRecurse))
    {
      return V;
    }
      ...
      if (Op0->getType ()->isIntOrIntVectorTy (1))
        {
          ...
          if (match (Op0, m_Select (m_Specific (Op1), m_One (), m_Value ())))
            return Op0;
        }
      // If the operation is with the result of a select instruction, check
      // whether operating on either branch of the select always yields the
  ...
  const APInt *C1;
  const APInt *C2;
  if (match (Op0, m_And (m_Value (A), m_APInt (C1))) && match (Op1, m_And (m_Value (B), m_APInt (C2))))
    {
      if (*C1 == ~*C2)
        {
          // (A & C1)|(B & C2)
...
/// If not, this returns null.
static Value *
SimplifyXorInst (Value *Op0, Value *Op1, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  auto foldAndOrNot = [] (Value *X, Value *Y) -> Value * {
    ...
    if (match (X, m_c_And (m_Not (m_Value (A)), m_Value (B))) && match (Y, m_c_Or (m_Specific (A), m_Specific (B))))
      {
        return A;
      }
    ...
    Value *NotA;
    if (match (X, m_c_Or (m_CombineAnd (m_NotForbidUndef (m_Value (A)), m_Value (NotA)), m_Value (B)))
        && match (Y, m_c_And (m_Specific (A), m_Specific (B))))
      {
        return NotA;
      }
  ...
  // Try some generic simplifications for associative operations.
  if (Value *V = SimplifyAssociativeBinOp (Instruction::Xor, Op0, Op1, Q, MaxRecurse))
    {
      return V;
    }
...
static Value *
ExtractEquivalentCondition (Value *V, CmpInst::Predicate Pred, Value *LHS, Value *RHS)
{
  ...
  if (Pred == CmpInst::getSwappedPredicate (Cmp->getPredicate ()) && LHS == CmpRHS && RHS == CmpLHS)
    {
      return Cmp;
    }
  return nullptr;
...
// reinstate this optimization.
static Constant *
computePointerICmp (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q)
{
  ...
  if (isa<ConstantPointerNull> (RHS) && ICmpInst::isEquality (Pred)
      && llvm::isKnownNonZero (LHS, DL, 0, nullptr, nullptr, nullptr, IIQ.UseInstrInfo))
    {
      return ConstantInt::get (GetCompareTy (LHS), static_cast<uint64_t> (!CmpInst::isTrueWhenEqual (Pred)));
    }
  ...
  switch (Pred)
    ...
    case CmpInst::ICMP_NE:
      ...
      // We can only handle unsigned relational comparisons because 'inbounds'
      // on a GEP only protects against unsigned wrapping.
  ...
  Constant *LHSOffset        = stripAndComputeConstantOffsets (DL, LHS, AllowNonInbounds);
  Constant *RHSOffset        = stripAndComputeConstantOffsets (DL, RHS, AllowNonInbounds);
  ...
  if (Pred == CmpInst::ICMP_EQ || Pred == CmpInst::ICMP_NE)
    {
      ...
      // scope of the problem by only looking at *static* allocas here. That
      // would cover the majority of allocas while significantly reducing the
      // likelihood of having an @llvm.stackrestore pop up in the middle.
      // However, it's not actually impossible for an @llvm.stackrestore to pop
      // up in the middle of an entry block. Also, if we have a block that's
      // not attached to a function, we can't tell if it's "static" under the
      // current definition. Theoretically, this problem could be fixed by
      // creating a new kind of instruction kind specifically for static
      // allocas. Such a new instruction could be required to be at the top of
      // the entry block, thus preventing it from being subject to a
      // @llvm.stackrestore. Instcombine could even convert regular allocas
      // into these special allocas. It'd be nifty. However, until then, this
      // problem remains open.
      //
      // So, we'll assume that two non-empty allocas have different addresses
      // for now.
      //
      // With all that, if the offsets are within the bounds of their
      // allocations (and not one-past-the-end! so we can't use inbounds!), and
      // their allocations aren't the same, the pointers are not equal.
      //
      // Note that it's not necessary to check for LHS being a global variable
      // address, due to canonicalization and constant folding.
      if (isa<AllocaInst> (LHS) && (isa<AllocaInst> (RHS) || isa<GlobalVariable> (RHS)))
        {
          ...
          uint64_t       RHSSize;
          ObjectSizeOpts Opts;
          Opts.NullIsUnknownSize = NullPointerIsDefined (cast<AllocaInst> (LHS)->getFunction ());
          if ((LHSOffsetCI != nullptr) && (RHSOffsetCI != nullptr) && getObjectSize (LHS, LHSSize, DL, TLI, Opts)
              && getObjectSize (RHS, RHSSize, DL, TLI, Opts))
            {
              const APInt &LHSOffsetValue = LHSOffsetCI->getValue ();
              const APInt &RHSOffsetValue = RHSOffsetCI->getValue ();
              if (!LHSOffsetValue.isNegative () && !RHSOffsetValue.isNegative () && LHSOffsetValue.ult (LHSSize)
          ...
          // Repeat the above check but this time without depending on
          // DataLayout or being able to compute a precise size.
          if (!cast<PointerType> (LHS->getType ())->isEmptyTy () && !cast<PointerType> (RHS->getType ())->isEmptyTy ()
      ...
      SmallVector<const Value *, 8> LHSUObjs;
      SmallVector<const Value *, 8> RHSUObjs;
      getUnderlyingObjects (LHS, LHSUObjs);
      getUnderlyingObjects (RHS, RHSUObjs);

      // Is the set of underlying objects all noalias calls?
      auto IsNAC = [] (ArrayRef<const Value *> Objects) { return all_of (Objects, isNoAliasCall); };

      // Is the set of underlying objects all things which must be disjoint
      // from noalias calls. For allocas, we consider only static ones (dynamic
      ...
      auto IsAllocDisjoint = [] (ArrayRef<const Value *> Objects) {
        return all_of (Objects, [] (const Value *V) {
          if (const AllocaInst *AI = dyn_cast<AllocaInst> (V))
            {
              return (AI->getParent () != nullptr) && (AI->getFunction () != nullptr) && AI->isStaticAlloca ();
            }
          if (const GlobalValue *GV = dyn_cast<GlobalValue> (V))
            {
              return (GV->hasLocalLinkage () || GV->hasHiddenVisibility () || GV->hasProtectedVisibility ()
      ...
      if ((IsNAC (LHSUObjs) && IsAllocDisjoint (RHSUObjs)) || (IsNAC (RHSUObjs) && IsAllocDisjoint (LHSUObjs)))
        {
          return ConstantInt::get (GetCompareTy (LHS), static_cast<uint64_t> (!CmpInst::isTrueWhenEqual (Pred)));
        }
      ...
      Value *MI = nullptr;
      if (isAllocLikeFn (LHS, TLI) && llvm::isKnownNonZero (RHS, DL, 0, nullptr, CxtI, DT))
        {
          MI = LHS;
        }
      else if (isAllocLikeFn (RHS, TLI) && llvm::isKnownNonZero (LHS, DL, 0, nullptr, CxtI, DT))
        {
          MI = RHS;
        }
      // FIXME: We should also fold the compare when the pointer escapes, but
      // the compare dominates the pointer escape
      if ((MI != nullptr) && !PointerMayBeCaptured (MI, true, true))
        {
          return ConstantInt::get (GetCompareTy (LHS), static_cast<uint64_t> (CmpInst::isFalseWhenEqual (Pred)));
        }
    }
...
static Value *
simplifyICmpOfBools (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q)
{
...
static Value *
simplifyICmpWithConstant (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const InstrInfoQuery &IIQ)
{
  ...
  ConstantRange LHS_CR = computeConstantRange (LHS, CmpInst::isSigned (Pred), IIQ.UseInstrInfo);
  if (!LHS_CR.isFullSet ())
    {
  ...
  if (ICmpInst::isEquality (Pred)
      && ((match (LHS, m_NUWMul (m_Value (), m_APIntAllowUndef (MulC))) && *MulC != 0 && C->urem (*MulC) != 0)
          || (match (LHS, m_NSWMul (m_Value (), m_APIntAllowUndef (MulC))) && *MulC != 0 && C->srem (*MulC) != 0)))
    {
      return ConstantInt::get (ITy, static_cast<uint64_t> (Pred == ICmpInst::ICMP_NE));
    }
...
static Value *
simplifyICmpWithBinOpOnLHS (CmpInst::Predicate Pred, BinaryOperator *LBO, Value *RHS, const SimplifyQuery &Q,
                            unsigned MaxRecurse)
{
  Type *ITy = GetCompareTy (RHS); // The return type.
  ...
  // x udiv y <=u x --> true.
  // x udiv y >u  x --> false.
  if (match (LBO, m_LShr (m_Specific (RHS), m_Value ())) || match (LBO, m_UDiv (m_Specific (RHS), m_Value ())))
    {
      // icmp pred (X op Y), X
      if (Pred == ICmpInst::ICMP_UGT)
        {
  ...
  const APInt *C1;
  const APInt *C2;
  if ((match (LBO, m_UDiv (m_Mul (m_Specific (RHS), m_APInt (C1)), m_APInt (C2))) && C1->ule (*C2))
...
//
static bool
trySimplifyICmpWithAdds (CmpInst::Predicate Pred, Value *LHS, Value *RHS)
{
  ...
  const APInt *C1;
  const APInt *C2;
  if (!match (LHS, m_c_Add (m_Value (X), m_APInt (C1))) || !match (RHS, m_c_Add (m_Specific (X), m_APInt (C2))))
    {
      return false;
    }

  return (C1->slt (*C2) && C1->isNonNegative ()) || (C2->slt (*C1) && C1->isNonPositive ());
}

static Value *simplifyICmpWithBinOp (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q,
                                     unsigned MaxRecurse);

/// Try hard to fold icmp with zero RHS because this is a common case.
static Value *
simplifyICmpWithZero (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q)
{
  if (!match (RHS, m_Zero ()))
    {
      return nullptr;
    ...
    case ICmpInst::ICMP_SLE:
      {
        ...
        if (LHSKnown.isNonNegative () && isKnownNonZero (LHS, Q.DL, 0, Q.AC, Q.CxtI, Q.DT))
          {
            return getFalse (ITy);
          }
        break;
...
static Value *
simplifyICmpWithMinMax (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  Type              *ITy = GetCompareTy (LHS); // The return type.
  Value             *A;
  Value             *B;
  ...
  else if (match (RHS, m_SMax (m_Value (A), m_Value (B))) && (A == LHS || B == LHS))
    {
      if (A != LHS)
        {
  ...
  else if (match (LHS, m_SMin (m_Value (A), m_Value (B))) && (A == RHS || B == RHS))
    {
      if (A != RHS)
        {
  ...
  else if (match (RHS, m_SMin (m_Value (A), m_Value (B))) && (A == LHS || B == LHS))
    {
      if (A != LHS)
        {
  ...
  if (P != CmpInst::BAD_ICMP_PREDICATE)
    {
      ...
      switch (P)
        ...
        case CmpInst::ICMP_SLE:
          // Equivalent to "A EqP B".  This may be the same as the condition
        ...
        case CmpInst::ICMP_SGT:
          {
            ...
            // Equivalent to "A InvEqP B".  This may be the same as the
            // condition tested in the max/min; if so, we can just return that.
  ...
  else if (match (RHS, m_UMax (m_Value (A), m_Value (B))) && (A == LHS || B == LHS))
    {
      if (A != LHS)
        {
  ...
  else if (match (LHS, m_UMin (m_Value (A), m_Value (B))) && (A == RHS || B == RHS))
    {
      if (A != RHS)
        {
  ...
  else if (match (RHS, m_UMin (m_Value (A), m_Value (B))) && (A == LHS || B == LHS))
    {
      if (A != LHS)
        {
  ...
  if (match (LHS, m_UMin (m_Value (), m_Value ())) || match (LHS, m_SMin (m_Value (), m_Value ())))
    {
      std::swap (LHS, RHS);
  ...
  Value *C;
  Value *D;
  if (match (LHS, m_SMax (m_Value (A), m_Value (B))) && match (RHS, m_SMin (m_Value (C), m_Value (D)))
      && (A == C || A == D || B == C || B == D))
    {
  ...
  else if (match (LHS, m_UMax (m_Value (A), m_Value (B))) && match (RHS, m_UMin (m_Value (C), m_Value (D)))
           && (A == C || A == D || B == C || B == D))
    {
...
static Value *
simplifyICmpWithDominatingAssume (CmpInst::Predicate Predicate, Value *LHS, Value *RHS, const SimplifyQuery &Q)
{
  // Gracefully handle instructions that have not been inserted yet.
  if ((Q.AC == nullptr) || (Q.CxtI == nullptr) || (Q.CxtI->getParent () == nullptr))
    {
      return nullptr;
    }
      ...
      for (auto &AssumeVH : Q.AC->assumptionsFor (AssumeBaseOp))
        {
          ...
          if (Optional<bool> Imp = isImpliedCondition (Assume->getArgOperand (0), Predicate, LHS, RHS, Q.DL))
            {
              if (isValidAssumeForContext (Assume, Q.CxtI, Q.DT))
                {
                  return ConstantInt::get (GetCompareTy (LHS), static_cast<uint64_t> (*Imp));
                }
            }
        }
    }
...
/// If not, this returns null.
static Value *
SimplifyICmpInst (unsigned Predicate, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  if (LHS == RHS || Q.isUndefValue (RHS))
    {
      return ConstantInt::get (ITy, static_cast<uint64_t> (CmpInst::isTrueWhenEqual (Pred)));
    }
  ...
  if (isa<Instruction> (RHS) && isa<Instruction> (LHS))
    {
      ...
      if ((Q.IIQ.getMetadata (RHS_Instr, LLVMContext::MD_range) != nullptr)
          && (Q.IIQ.getMetadata (LHS_Instr, LLVMContext::MD_range) != nullptr))
        {
          auto RHS_CR = getConstantRangeFromMetadata (*RHS_Instr->getMetadata (LLVMContext::MD_range));
          auto LHS_CR = getConstantRangeFromMetadata (*LHS_Instr->getMetadata (LLVMContext::MD_range));
  ...
  if (isa<CastInst> (LHS) && (isa<Constant> (RHS) || isa<CastInst> (RHS)))
    {
      ...
      // input if the integer type is the same size as the pointer type.
          ...
          if (Constant *RHSC = dyn_cast<Constant> (RHS))
            {
              ...
              if (Value *V = SimplifyICmpInst (Pred, SrcOp, ConstantExpr::getIntToPtr (RHSC, SrcTy), Q, MaxRecurse - 1))
                {
                  return V;
                }
            }
              ...
              if (RI->getOperand (0)->getType () == SrcTy)
                {
                  ...
                  if (Value *V = SimplifyICmpInst (Pred, SrcOp, RI->getOperand (0), Q, MaxRecurse - 1))
                    {
                      return V;
                    }
                }
      ...
      if (isa<ZExtInst> (LHS))
        {
          // Turn icmp (zext X), (zext Y) into a compare of X and Y if they
          // have the same type.
          if (ZExtInst *RI = dyn_cast<ZExtInst> (RHS))
            {
              if ((MaxRecurse != 0u) && SrcTy == RI->getOperand (0)->getType ())
                {
                  // Compare X and Y.  Note that signed predicates become
                  // unsigned.
                  if (Value *V = SimplifyICmpInst (ICmpInst::getUnsignedPredicate (Pred), SrcOp, RI->getOperand (0), Q,
                                                   MaxRecurse - 1))
                    {
                      return V;
                    }
          ...
          // Turn icmp (zext X), Cst into a compare of X and Cst if Cst is
          // extended too.  If not, then try to deduce the result of the
          // comparison.
          else if (ConstantInt *CI = dyn_cast<ConstantInt> (RHS))
            {
              // Compute the constant that would happen if we truncated to
              // SrcTy then reextended to DstTy.
              Constant *Trunc = ConstantExpr::getTrunc (CI, SrcTy);
              Constant *RExt  = ConstantExpr::getCast (CastInst::ZExt, Trunc, DstTy);

              // If the re-extended constant didn't change then this is
              // effectively also a case of comparing two zero-extended values.
              if (RExt == CI && (MaxRecurse != 0u))
                {
                  if (Value *V
                      = SimplifyICmpInst (ICmpInst::getUnsignedPredicate (Pred), SrcOp, Trunc, Q, MaxRecurse - 1))
                    {
                      return V;
                    }
                }

              // Otherwise the upper bits of LHS are zero while RHS has a
              // non-zero bit there.  Use this to work out the result of the
              // comparison.
              if (RExt != CI)
                {
                  switch (Pred)
                    ...
                    // LHS is non-negative.  If RHS is negative then LHS >s
                    // LHS.  If RHS is non-negative then LHS <s RHS.
                    case ICmpInst::ICMP_SGT:
                    case ICmpInst::ICMP_SGE:
                      return CI->getValue ().isNegative () ? ConstantInt::getTrue (CI->getContext ())
                                                           : ConstantInt::getFalse (CI->getContext ());

                    case ICmpInst::ICMP_SLT:
                    case ICmpInst::ICMP_SLE:
                      return CI->getValue ().isNegative () ? ConstantInt::getFalse (CI->getContext ())
      ...
      if (isa<SExtInst> (LHS))
        {
          // Turn icmp (sext X), (sext Y) into a compare of X and Y if they
          // have the same type.
          ...
          // Turn icmp (sext X), Cst into a compare of X and Cst if Cst is
          // extended too.  If not, then try to deduce the result of the
          // comparison.
          else if (ConstantInt *CI = dyn_cast<ConstantInt> (RHS))
            {
              ...
              // If the re-extended constant didn't change then this is
              // effectively also a case of comparing two sign-extended values.
              if (RExt == CI && (MaxRecurse != 0u))
                {
              ...
              // Otherwise the upper bits of LHS are all equal, while RHS has
              // varying bits there.  Use this to work out the result of the
              // comparison.
              if (RExt != CI)
                {
                  switch (Pred)
                    ...
                    // If RHS is non-negative then LHS <s RHS.  If RHS is
                    ...
                    // If LHS is non-negative then LHS <u RHS.  If LHS is
                    ...
                    case ICmpInst::ICMP_UGE:
                      ...
                      if (MaxRecurse != 0u)
                        {
                          if (Value *V = SimplifyICmpInst (ICmpInst::ICMP_SLT, SrcOp, Constant::getNullValue (SrcTy), Q,
                                                           MaxRecurse - 1))
                            {
                              return V;
                            }
                    ...
                    case ICmpInst::ICMP_ULE:
                      ...
                      if (MaxRecurse != 0u)
                        {
                          if (Value *V = SimplifyICmpInst (ICmpInst::ICMP_SGE, SrcOp, Constant::getNullValue (SrcTy), Q,
                                                           MaxRecurse - 1))
                            {
                              return V;
                            }
  ...
  if (auto *CLHS = dyn_cast<PtrToIntOperator> (LHS))
    {
      if (auto *CRHS = dyn_cast<PtrToIntOperator> (RHS))
        {
          if (Q.DL.getTypeSizeInBits (CLHS->getPointerOperandType ()) == Q.DL.getTypeSizeInBits (CLHS->getType ())
              && Q.DL.getTypeSizeInBits (CRHS->getPointerOperandType ()) == Q.DL.getTypeSizeInBits (CRHS->getType ()))
            {
              if (auto *C = computePointerICmp (Pred, CLHS->getPointerOperand (), CRHS->getPointerOperand (), Q))
                {
                  return C;
                }
            }
  ...
  // If the comparison is with the result of a select instruction, check
  // whether comparing with either branch of the select always yields the same
  // value.
...
/// If not, this returns null.
static Value *
SimplifyFCmpInst (unsigned Predicate, Value *LHS, Value *RHS, FastMathFlags FMF, const SimplifyQuery &Q,
                  unsigned MaxRecurse)
{
  CmpInst::Predicate Pred = (CmpInst::Predicate)Predicate;
  ...
  if (Pred == FCmpInst::FCMP_UNO || Pred == FCmpInst::FCMP_ORD)
    {
      if (FMF.noNaNs () || (isKnownNeverNaN (LHS, Q.TLI) && isKnownNeverNaN (RHS, Q.TLI)))
        {
          return ConstantInt::get (RetTy, static_cast<uint64_t> (Pred == FCmpInst::FCMP_ORD));
        }
    }
  ...
  if (match (RHS, m_NaN ()))
    {
      return ConstantInt::get (RetTy, static_cast<uint64_t> (CmpInst::isUnordered (Pred)));
    }
  ...
  if (Q.isUndefValue (LHS) || Q.isUndefValue (RHS))
    {
      // Choosing NaN for the undef will always make unordered comparison
      // succeed and ordered comparison fail.
      return ConstantInt::get (RetTy, static_cast<uint64_t> (CmpInst::isUnordered (Pred)));
    }
  ...
  if (match (RHS, m_APFloat (C)))
    {
      ...
      if (C->isInfinity ())
        {
          ...
          // LHS == Inf || LHS == NaN
          if (Pred == FCmpInst::FCMP_UEQ && isKnownNeverInfinity (LHS, Q.TLI) && isKnownNeverNaN (LHS, Q.TLI))
            {
              return getFalse (RetTy);
            }
          // LHS != Inf && LHS != NaN
          if (Pred == FCmpInst::FCMP_ONE && isKnownNeverInfinity (LHS, Q.TLI) && isKnownNeverNaN (LHS, Q.TLI))
            {
              return getTrue (RetTy);
            }
        }
      ...
      const APFloat *C2;
      if ((match (LHS, m_Intrinsic<Intrinsic::minnum> (m_Value (), m_APFloat (C2))) && *C2 < *C)
          || (match (LHS, m_Intrinsic<Intrinsic::maxnum> (m_Value (), m_APFloat (C2))) && *C2 > *C))
        {
          bool IsMaxNum = cast<IntrinsicInst> (LHS)->getIntrinsicID () == Intrinsic::maxnum;
          // The ordered relationship and minnum/maxnum guarantee that we do
          // not have NaN constants, so ordered/unordered preds are handled the
          // same.
          switch (Pred)
            {
            case FCmpInst::FCMP_OEQ:
            ...
            case FCmpInst::FCMP_UGT:
              ...
              // maxnum(X, GreaterC) >= C --> true
              // maxnum(X, GreaterC) >  C --> true
              return ConstantInt::get (RetTy, static_cast<uint64_t> (IsMaxNum));
            case FCmpInst::FCMP_OLE:
            case FCmpInst::FCMP_ULE:
            case FCmpInst::FCMP_OLT:
...
static Value *
simplifyWithOpReplaced (Value *V, Value *Op, Value *RepOp, const SimplifyQuery &Q, bool AllowRefinement,
                        unsigned MaxRecurse)
{
  ...
  transform (I->operands (), NewOps.begin (), [&] (Value *V) { return V == Op ? RepOp : V; });

  if (!AllowRefinement)
    {
      // General InstSimplify functions may refine the result, e.g. by
      // returning a constant for a potentially poison value. To avoid this,
      ...
      if (auto *BO = dyn_cast<BinaryOperator> (I))
        {
          ...
          // x & x -> x, x | x -> x
          if ((Opcode == Instruction::And || Opcode == Instruction::Or) && NewOps[0] == NewOps[1])
            {
              return NewOps[0];
            }
        }
      ...
      if (auto *GEP = dyn_cast<GetElementPtrInst> (I))
        {
          // getelementptr x, 0 -> x
          if (NewOps.size () == 2 && match (NewOps[1], m_Zero ()) && !GEP->isInBounds ())
            {
              return NewOps[0];
            }
        }
  ...
  else if (MaxRecurse != 0u)
    {
      ...
      // dominate %div. To ensure a consistent return value contract, we make
      // sure that this case returns nullptr as well.
      auto PreventSelfSimplify = [V] (Value *Simplified) { return Simplified != V ? Simplified : nullptr; };

      if (auto *B = dyn_cast<BinaryOperator> (I))
        {
          return PreventSelfSimplify (SimplifyBinOp (B->getOpcode (), NewOps[0], NewOps[1], Q, MaxRecurse - 1));
        }

      if (CmpInst *C = dyn_cast<CmpInst> (I))
        {
          return PreventSelfSimplify (SimplifyCmpInst (C->getPredicate (), NewOps[0], NewOps[1], Q, MaxRecurse - 1));
        }

      if (auto *GEP = dyn_cast<GetElementPtrInst> (I))
        {
          return PreventSelfSimplify (SimplifyGEPInst (GEP->getSourceElementType (), NewOps[0],
                                                       makeArrayRef (NewOps).slice (1), GEP->isInBounds (), Q,
                                                       MaxRecurse - 1));
        }

      if (isa<SelectInst> (I))
        {
          return PreventSelfSimplify (SimplifySelectInst (NewOps[0], NewOps[1], NewOps[2], Q, MaxRecurse - 1));
        }
  ...
  if (CmpInst *C = dyn_cast<CmpInst> (I))
    {
      return ConstantFoldCompareInstOperands (C->getPredicate (), ConstOps[0], ConstOps[1], Q.DL, Q.TLI);
    }
...
static Value *
simplifySelectBitTest (Value *TrueVal, Value *FalseVal, Value *X, const APInt *Y, bool TrueWhenUnset)
{
  const APInt *C;

  // (X & Y) == 0 ? X & ~Y : X  --> X
  // (X & Y) != 0 ? X & ~Y : X  --> X & ~Y
  if (FalseVal == X && match (TrueVal, m_And (m_Specific (X), m_APInt (C))) && *Y == ~*C)
    {
  ...
  // (X & Y) == 0 ? X : X & ~Y  --> X & ~Y
  // (X & Y) != 0 ? X : X & ~Y  --> X
  if (TrueVal == X && match (FalseVal, m_And (m_Specific (X), m_APInt (C))) && *Y == ~*C)
    {
      return TrueWhenUnset ? FalseVal : TrueVal;
    }
  ...
  if (Y->isPowerOf2 ())
    {
      // (X & Y) == 0 ? X | Y : X  --> X | Y
      // (X & Y) != 0 ? X | Y : X  --> X
      if (FalseVal == X && match (TrueVal, m_Or (m_Specific (X), m_APInt (C))) && *Y == *C)
        {
      ...
      // (X & Y) == 0 ? X : X | Y  --> X
      // (X & Y) != 0 ? X : X | Y  --> X | Y
      if (TrueVal == X && match (FalseVal, m_Or (m_Specific (X), m_APInt (C))) && *Y == *C)
        {
          return TrueWhenUnset ? TrueVal : FalseVal;
        }
    }
...
/// eq/ne.
static Value *
simplifySelectWithFakeICmpEq (Value *CmpLHS, Value *CmpRHS, ICmpInst::Predicate Pred, Value *TrueVal, Value *FalseVal)
{
  Value *X;
  APInt  Mask;
  ...
  return simplifySelectBitTest (TrueVal, FalseVal, X, &Mask, Pred == ICmpInst::ICMP_EQ);
}

/// Try to simplify a select instruction when its condition operand is an
/// integer comparison.
static Value *
simplifySelectWithICmpCond (Value *CondVal, Value *TrueVal, Value *FalseVal, const SimplifyQuery &Q,
                            unsigned MaxRecurse)
{
  ...
  if (TrueVal->getType ()->isIntOrIntVectorTy ())
    {
      Value              *X;
      Value              *Y;
      SelectPatternFlavor SPF = matchDecomposedSelectPattern (cast<ICmpInst> (CondVal), TrueVal, FalseVal, X, Y).Flavor;
      if (SelectPatternResult::isMinOrMax (SPF) && Pred == getMinMaxPred (SPF))
        {
          APInt LimitC = getMinMaxLimit (getInverseMinMaxFlavor (SPF), X->getType ()->getScalarSizeInBits ());
  ...
  if (Pred == ICmpInst::ICMP_EQ && match (CmpRHS, m_Zero ()))
    {
      ...
      // We do not allow this transform for the general funnel shift case
      // because that would not preserve the poison safety of the original
      // code.
      auto isRotate = m_CombineOr (m_FShl (m_Value (X), m_Deferred (X), m_Value (ShAmt)),
                                   m_FShr (m_Value (X), m_Deferred (X), m_Value (ShAmt)));
      // (ShAmt == 0) ? X : fshl(X, X, ShAmt) --> fshl(X, X, ShAmt)
      // (ShAmt == 0) ? X : fshr(X, X, ShAmt) --> fshr(X, X, ShAmt)
      if (match (FalseVal, isRotate) && TrueVal == X && CmpLHS == ShAmt && Pred == ICmpInst::ICMP_EQ)
        {
          return FalseVal;
        }
  ...
  if (Value *V = simplifySelectWithFakeICmpEq (CmpLHS, CmpRHS, Pred, TrueVal, FalseVal))
    {
      return V;
    }
  ...
  // Note that the equivalence/replacement opportunity does not hold for
  // vectors because each element of a vector select is chosen independently.
...
static Value *
simplifySelectWithFCmp (Value *Cond, Value *T, Value *F, const SimplifyQuery &Q)
{
  ...
  bool           HasNoSignedZeros = (Q.CxtI != nullptr) && isa<FPMathOperator> (Q.CxtI) && Q.CxtI->hasNoSignedZeros ();
  const APFloat *C;
...
/// duplication.
static Value *
simplifyICmpWithBinOp (CmpInst::Predicate Pred, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  if ((MaxRecurse != 0u) && ((LBO != nullptr) || (RBO != nullptr)))
    {
      ...
      if ((LBO != nullptr) && LBO->getOpcode () == Instruction::Add)
        {
          A = LBO->getOperand (0);
          B = LBO->getOperand (1);
          NoLHSWrapProblem
              = ICmpInst::isEquality (Pred)
                || (CmpInst::isUnsigned (Pred) && Q.IIQ.hasNoUnsignedWrap (cast<OverflowingBinaryOperator> (LBO)))
                || (CmpInst::isSigned (Pred) && Q.IIQ.hasNoSignedWrap (cast<OverflowingBinaryOperator> (LBO)));
        }
      if ((RBO != nullptr) && RBO->getOpcode () == Instruction::Add)
        {
          C = RBO->getOperand (0);
          D = RBO->getOperand (1);
          NoRHSWrapProblem
              = ICmpInst::isEquality (Pred)
                || (CmpInst::isUnsigned (Pred) && Q.IIQ.hasNoUnsignedWrap (cast<OverflowingBinaryOperator> (RBO)))
                || (CmpInst::isSigned (Pred) && Q.IIQ.hasNoSignedWrap (cast<OverflowingBinaryOperator> (RBO)));
        }
      ...
      if ((A == RHS || B == RHS) && NoLHSWrapProblem)
        {
          if (Value *V
              = SimplifyICmpInst (Pred, A == RHS ? B : A, Constant::getNullValue (RHS->getType ()), Q, MaxRecurse - 1))
            {
              return V;
            }
        }
      ...
      if ((C == LHS || D == LHS) && NoRHSWrapProblem)
        {
          if (Value *V
              = SimplifyICmpInst (Pred, Constant::getNullValue (LHS->getType ()), C == LHS ? D : C, Q, MaxRecurse - 1))
            {
              return V;
            }
        }

      // icmp (X+Y), (X+Z) -> icmp Y,Z for equalities or if there is no
      // overflow.
      bool CanSimplify = (NoLHSWrapProblem && NoRHSWrapProblem) || trySimplifyICmpWithAdds (Pred, LHS, RHS);
      if ((A != nullptr) && (C != nullptr) && (A == C || A == D || B == C || B == D) && CanSimplify)
        {
  ...
  if (RBO != nullptr)
    {
      if (Value *V = simplifyICmpWithBinOpOnLHS (ICmpInst::getSwappedPredicate (Pred), RBO, LHS, Q, MaxRecurse))
        {
          return V;
        }
    }
  ...
  //   (C2 << X) != C --> true
  const APInt *C;
  if (match (LHS, m_Shl (m_Power2 (), m_Value ())) && match (RHS, m_APIntAllowUndef (C)) && !C->isPowerOf2 ())
    {
  ...
  if ((MaxRecurse != 0u) && (LBO != nullptr) && (RBO != nullptr) && LBO->getOpcode () == RBO->getOpcode ()
      && LBO->getOperand (1) == RBO->getOperand (1))
    {
      switch (LBO->getOpcode ())
        ...
        case Instruction::LShr:
          ...
          if (Value *V = SimplifyICmpInst (Pred, LBO->getOperand (0), RBO->getOperand (0), Q, MaxRecurse - 1))
            {
              return V;
            }
          break;
...
/// If not, this returns null.
static Value *
SimplifySelectInst (Value *Cond, Value *TrueVal, Value *FalseVal, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  ...
  assert (Cond->getType ()->isIntOrIntVectorTy (1) && "Select must have bool or bool vector condition");
  assert (TrueVal->getType () == FalseVal->getType () && "Select must have same types for true/false ops");
  ...
  if (isa<PoisonValue> (TrueVal)
      || (Q.isUndefValue (TrueVal) && isGuaranteedNotToBePoison (FalseVal, Q.AC, Q.CxtI, Q.DT)))
    {
      return FalseVal;
    }
  // select ?, X, poison -> X
  ...
  if (isa<PoisonValue> (FalseVal)
      || (Q.isUndefValue (FalseVal) && isGuaranteedNotToBePoison (TrueVal, Q.AC, Q.CxtI, Q.DT)))
    {
      return TrueVal;
    }
  ...
  Constant *TrueC;
  Constant *FalseC;
  if (isa<FixedVectorType> (TrueVal->getType ()) && match (TrueVal, m_Constant (TrueC))
      && match (FalseVal, m_Constant (FalseC)))
    {
      unsigned                    NumElts = cast<FixedVectorType> (TrueC->getType ())->getNumElements ();
      ...
      for (unsigned i = 0; i != NumElts; ++i)
        {
          ...
          // If the elements match (undef or not), that value is the result. If
          // only one element is undef, choose the defined element as the safe
          // result.
          if (TEltC == FEltC)
            {
              NewC.push_back (TEltC);
            }
          else if (isa<PoisonValue> (TEltC) || (Q.isUndefValue (TEltC) && isGuaranteedNotToBePoison (FEltC)))
            {
              NewC.push_back (FEltC);
            }
          else if (isa<PoisonValue> (FEltC) || (Q.isUndefValue (FEltC) && isGuaranteedNotToBePoison (TEltC)))
            {
              NewC.push_back (TEltC);
            }
          else
  ...
  if (Value *V = simplifySelectWithICmpCond (Cond, TrueVal, FalseVal, Q, MaxRecurse))
    {
      return V;
    }
...
/// If not, this returns null.
static Value *
SimplifyGEPInst (Type *SrcTy, Value *Ptr, ArrayRef<Value *> Indices, bool InBounds, const SimplifyQuery &Q,
                 unsigned /*unused*/)
{
  // The type of the GEP pointer operand.
  unsigned AS = cast<PointerType> (Ptr->getType ()->getScalarType ())->getAddressSpace ();
      ...
      for (Value *Op : Indices)
        {
          ...
          // pointers. All vector operands must have the same number of
          // elements.
  ...
  if (isa<PoisonValue> (Ptr) || any_of (Indices, [] (const auto *V) { return isa<PoisonValue> (V); }))
    {
  ...
  bool IsScalableVec = isa<ScalableVectorType> (SrcTy)
                       || any_of (Indices, [] (const Value *V) { return isa<ScalableVectorType> (V->getType ()); });
      ...
      if (!IsScalableVec && Ty->isSized ())
        {
          ...
          if (Indices[0]->getType ()->getScalarSizeInBits () == Q.DL.getPointerSizeInBits (AS))
            {
              auto CanSimplify = [GEPTy, &P, Ptr] () -> bool {
                return P->getType () == GEPTy && getUnderlyingObject (P) == getUnderlyingObject (Ptr);
              };
              // getelementptr V, (sub P, V) -> P if P points to a type of
              // size 1.
              if (TyAllocSize == 1
                  && match (Indices[0], m_Sub (m_PtrToInt (m_Value (P)), m_PtrToInt (m_Specific (Ptr))))
                  && CanSimplify ())
                {
                  return P;
                }

              // getelementptr V, (ashr (sub P, V), C) -> P if P points to a
              // type of size 1 << C.
              if (match (Indices[0],
                         m_AShr (m_Sub (m_PtrToInt (m_Value (P)), m_PtrToInt (m_Specific (Ptr))), m_ConstantInt (C)))
                  && TyAllocSize == 1ULL << C && CanSimplify ())
                {
                  return P;
                }

              // getelementptr V, (sdiv (sub P, V), C) -> P if P points to a
              // type of size C.
              if (match (Indices[0], m_SDiv (m_Sub (m_PtrToInt (m_Value (P)), m_PtrToInt (m_Specific (Ptr))),
  ...
  if (!IsScalableVec && Q.DL.getTypeAllocSize (LastType) == 1
      && all_of (Indices.drop_back (1), [] (Value *Idx) { return match (Idx, m_Zero ()); }))
    {
      unsigned IdxWidth = Q.DL.getIndexSizeInBits (Ptr->getType ()->getPointerAddressSpace ());
      if (Q.DL.getTypeSizeInBits (Indices.back ()->getType ()) == IdxWidth)
        {
          APInt  BasePtrOffset (IdxWidth, 0);
          Value *StrippedBasePtr = Ptr->stripAndAccumulateInBoundsConstantOffsets (Q.DL, BasePtrOffset);

          // Avoid creating inttoptr of zero here: While LLVMs treatment of
          // inttoptr is generally conservative, this particular case is folded
          // to a null pointer, which will have incorrect provenance.

          // gep (gep V, C), (sub 0, V) -> C
          if (match (Indices.back (), m_Sub (m_Zero (), m_PtrToInt (m_Specific (StrippedBasePtr))))
          ...
          if (match (Indices.back (), m_Xor (m_PtrToInt (m_Specific (StrippedBasePtr)), m_AllOnes ()))
              && !BasePtrOffset.isOne ())
            {
  ...
  if (!isa<Constant> (Ptr) || !all_of (Indices, [] (Value *V) { return isa<Constant> (V); }))
    {
      return nullptr;
    }

  auto *CE = ConstantExpr::getGetElementPtr (SrcTy, cast<Constant> (Ptr), Indices, InBounds);
...
/// If not, this returns null.
static Value *
SimplifyInsertValueInst (Value *Agg, Value *Val, ArrayRef<unsigned> Idxs, const SimplifyQuery &Q, unsigned /*unused*/)
{
  ...
  if (ExtractValueInst *EV = dyn_cast<ExtractValueInst> (Val))
    {
      if (EV->getAggregateOperand ()->getType () == Agg->getType () && EV->getIndices () == Idxs)
        {
...
static Value *
SimplifyInsertElementInst (Value *Vec, Value *Val, Value *Idx, const SimplifyQuery &Q)
{
  ...
  if (isa<PoisonValue> (Val) || (Q.isUndefValue (Val) && isGuaranteedNotToBePoison (Vec)))
    {
      return Vec;
    }

  // If we are extracting a value from a vector, then inserting it into the
  // same place, that's the input vector: insertelt Vec, (extractelt Vec, Idx),
  // Idx --> Vec
...
/// If not, this returns null.
static Value *
SimplifyExtractValueInst (Value *Agg, ArrayRef<unsigned> Idxs, const SimplifyQuery & /*unused*/, unsigned /*unused*/)
{
  ...
  for (auto *IVI = dyn_cast<InsertValueInst> (Agg); IVI != nullptr;
       IVI       = dyn_cast<InsertValueInst> (IVI->getAggregateOperand ()))
    {
      ...
      if (InsertValueIdxs.slice (0, NumCommonIdxs) == Idxs.slice (0, NumCommonIdxs))
        {
...
/// If not, this returns null.
static Value *
SimplifyExtractElementInst (Value *Vec, Value *Idx, const SimplifyQuery &Q, unsigned /*unused*/)
{
...
static Value *
SimplifyPHINode (PHINode *PN, ArrayRef<Value *> IncomingValues, const SimplifyQuery &Q)
{
...
static Value *
SimplifyCastInst (unsigned CastOpc, Value *Op, Type *Ty, const SimplifyQuery &Q, unsigned MaxRecurse)
{
      ...
      if (Src->getType () == Ty)
        {
          ...
          Type *SrcIntPtrTy = SrcTy->isPtrOrPtrVectorTy () ? Q.DL.getIntPtrType (SrcTy) : nullptr;
          Type *MidIntPtrTy = MidTy->isPtrOrPtrVectorTy () ? Q.DL.getIntPtrType (MidTy) : nullptr;
          Type *DstIntPtrTy = DstTy->isPtrOrPtrVectorTy () ? Q.DL.getIntPtrType (DstTy) : nullptr;
          if (CastInst::isEliminableCastPair (FirstOp, SecondOp, SrcTy, MidTy, DstTy, SrcIntPtrTy, MidIntPtrTy,
                                              DstIntPtrTy)
              == Instruction::BitCast)
            {
              return Src;
...
static Value *
foldIdentityShuffles (int DestElt, Value *Op0, Value *Op1, int MaskVal, Value *RootVec, unsigned MaxRecurse)
{
  if ((MaxRecurse--) == 0u)
    {
      return nullptr;
  ...
  if (auto *SourceShuf = dyn_cast<ShuffleVectorInst> (SourceOp))
    {
      return foldIdentityShuffles (DestElt, SourceShuf->getOperand (0), SourceShuf->getOperand (1),
                                   SourceShuf->getMaskValue (RootElt), RootVec, MaxRecurse);
    }

  // TODO: Look through bitcasts? What if the bitcast changes the vector
  // element size?
  ...
  // Give up as soon as a source operand does not match the existing root
  // value.
  if (RootVec != SourceOp)
    {
      return nullptr;
...
static Value *
SimplifyShuffleVectorInst (Value *Op0, Value *Op1, ArrayRef<int> Mask, Type *RetTy, const SimplifyQuery &Q,
                           unsigned MaxRecurse)
{
  ...
  if (!Scalable && (Op0Const != nullptr) && (Op1Const == nullptr))
    {
      std::swap (Op0, Op1);
      ShuffleVectorInst::commuteShuffleMask (Indices, InVecEltCount.getKnownMinValue ());
    }
  ...
  Constant    *C;
  ConstantInt *IndexC;
  if (!Scalable && match (Op0, m_InsertElt (m_Value (), m_Constant (C), m_ConstantInt (IndexC))))
    {
      // Match a splat shuffle mask of the insert index allowing undef
      // elements.
      int InsertIndex = IndexC->getZExtValue ();
      if (all_of (Indices, [InsertIndex] (int MaskElt) { return MaskElt == InsertIndex || MaskElt == -1; }))
        {
  ...
  if (auto *OpShuf = dyn_cast<ShuffleVectorInst> (Op0))
    {
      if (Q.isUndefValue (Op1) && RetTy == InVecTy && is_splat (OpShuf->getShuffleMask ()))
        {
          return Op0;
        }
    }
  ...
  for (unsigned i = 0; i != MaskNumElts; ++i)
    {
      // Note that recursion is limited for each vector element, so if any
      // element exceeds the limit, this will fail to simplify.
      RootVec = foldIdentityShuffles (i, Op0, Op1, Indices[i], RootVec, MaxRecurse);

      // We can't replace a widening/narrowing shuffle with one of its
      // operands.
...
static Constant *
foldConstant (Instruction::UnaryOps Opcode, Value *&Op, const SimplifyQuery &Q)
{
  if (auto *C = dyn_cast<Constant> (Op))
    {
...
/// returns null.
static Value *
simplifyFNegInst (Value *Op, FastMathFlags FMF, const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
static Constant *
simplifyFPOp (ArrayRef<Value *> Ops, FastMathFlags FMF, const SimplifyQuery &Q, fp::ExceptionBehavior ExBehavior,
  ...
  for (Value *V : Ops)
    {
      ...
      // If this operation has 'nnan' or 'ninf' and at least 1 disallowed
      // operand (an undef operand can be chosen to be Nan/Inf), then the
      // result of this operation is poison.
...
/// returns null.
static Value *
SimplifyFAddInst (Value *Op0, Value *Op1, FastMathFlags FMF, const SimplifyQuery &Q, unsigned MaxRecurse,
                  ...
                  RoundingMode          Rounding   = RoundingMode::NearestTiesToEven)
{
  ...
  if (canIgnoreSNaN (ExBehavior, FMF)
      && (!canRoundingModeBe (Rounding, RoundingMode::TowardNegative) || FMF.noSignedZeros ()))
    {
  ...
  if (canIgnoreSNaN (ExBehavior, FMF))
    {
      if (match (Op1, m_PosZeroFP ()) && (FMF.noSignedZeros () || CannotBeNegativeZero (Op0, Q.TLI)))
        {
          return Op0;
        }
    }
  ...
  if (FMF.noNaNs ())
    {
      ...
      if (match (Op0, m_FNeg (m_Specific (Op1))) || match (Op1, m_FNeg (m_Specific (Op0))))
        {
          return ConstantFP::getNullValue (Op0->getType ());
        }
    }
  ...
  if (FMF.noSignedZeros () && FMF.allowReassoc ()
      && (match (Op0, m_FSub (m_Value (X), m_Specific (Op1))) || match (Op1, m_FSub (m_Value (X), m_Specific (Op0)))))
    {
      return X;
    }
...
static Value *SimplifyFSubInst (Value *Op0, Value *Op1, FastMathFlags FMF, const SimplifyQuery &Q,
                                unsigned MaxRecurse = 1000, fp::ExceptionBehavior ExBehavior = fp::ebIgnore,
                                RoundingMode Rounding = RoundingMode::NearestTiesToEven);

static Value *
SimplifyFMAFMul (Value *Op0, Value *Op1, FastMathFlags FMF, const SimplifyQuery &Q, unsigned MaxRecurse,
                 fp::ExceptionBehavior ExBehavior, RoundingMode Rounding)
{
  if (Constant *C = simplifyFPOp ({Op0, Op1}, FMF, Q, ExBehavior, Rounding))
    {
      return C;
  ...
  Value *X;
  if (Op0 == Op1 && match (Op0, m_Intrinsic<Intrinsic::sqrt> (m_Value (X))) && FMF.allowReassoc () && FMF.noNaNs ()
      && FMF.noSignedZeros ())
    {
      return X;
    }
...
static Value *
SimplifyFMulInst (Value *Op0, Value *Op1, FastMathFlags FMF, const SimplifyQuery &Q, unsigned MaxRecurse = 1000,
                  fp::ExceptionBehavior ExBehavior = fp::ebIgnore,
                  RoundingMode          Rounding   = RoundingMode::NearestTiesToEven)
{
  if (isDefaultFPEnvironment (ExBehavior, Rounding))
...
static Value *
SimplifyFDivInst (Value *Op0, Value *Op1, FastMathFlags FMF, const SimplifyQuery &Q, unsigned R = 1000,
                  fp::ExceptionBehavior ExBehavior = fp::ebIgnore,
                  RoundingMode          Rounding   = RoundingMode::NearestTiesToEven)
{
  if (isDefaultFPEnvironment (ExBehavior, Rounding))
  ...
  if (FMF.noNaNs ())
    {
      ...
      if (match (Op0, m_FNegNSZ (m_Specific (Op1))) || match (Op1, m_FNegNSZ (m_Specific (Op0))))
        {
          return ConstantFP::get (Op0->getType (), -1.0);
        }
    }
...
static Value *
SimplifyFRemInst (Value *Op0, Value *Op1, FastMathFlags FMF, const SimplifyQuery &Q, unsigned /*unused*/,
                  fp::ExceptionBehavior ExBehavior = fp::ebIgnore,
                  RoundingMode          Rounding   = RoundingMode::NearestTiesToEven)
{
  if (isDefaultFPEnvironment (ExBehavior, Rounding))
  ...
  // The constant match may include undef elements in a vector, so return a
  // full zero constant as the result.
  if (FMF.noNaNs ())
    {
      // +0 % X -> 0
...
/// If not, this returns null.
static Value *
simplifyUnOp (unsigned Opcode, Value *Op, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  switch (Opcode)
    {
...
static Value *
simplifyFPUnOp (unsigned Opcode, Value *Op, const FastMathFlags &FMF, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  switch (Opcode)
    {
    case Instruction::FNeg:
...
/// If not, this returns null.
static Value *
SimplifyBinOp (unsigned Opcode, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
  switch (Opcode)
    {
...
/// Try to use FastMathFlags when folding the result.
static Value *
SimplifyBinOp (unsigned Opcode, Value *LHS, Value *RHS, const FastMathFlags &FMF, const SimplifyQuery &Q,
               unsigned MaxRecurse)
{
  switch (Opcode)
    {
...
static Value *
SimplifyCmpInst (unsigned Predicate, Value *LHS, Value *RHS, const SimplifyQuery &Q, unsigned MaxRecurse)
{
...
static Value *
SimplifyRelativeLoad (Constant *Ptr, Constant *Offset, const DataLayout &DL)
{
  GlobalValue *PtrSym;
  APInt        PtrOffset;
  ...
  if ((OffsetConstInt == nullptr) || OffsetConstInt->getType ()->getBitWidth () > 64)
    {
      return nullptr;
    }
  ...
  Constant *C      = ConstantExpr::getGetElementPtr (Int32Ty, ConstantExpr::getBitCast (Ptr, Int32PtrTy),
  ...
  if ((LoadedLHS == nullptr) || LoadedLHS->getOpcode () != Instruction::PtrToInt)
    {
      return nullptr;
    }
  ...
  if (!IsConstantOffsetFromGlobal (LoadedRHS, LoadedRHSSym, LoadedRHSOffset, DL) || PtrSym != LoadedRHSSym
      || PtrOffset != LoadedRHSOffset)
    {
      return nullptr;
    }
...
static Value *
simplifyUnaryIntrinsic (Function *F, Value *Op0, const SimplifyQuery &Q)
{
  ...
  switch (IID)
    ...
    case Intrinsic::ctpop:
      {
        // If everything but the lowest bit is zero, that bit is the pop-count.
        // Ex: ctpop(and X, 1) --> and X, 1
        unsigned BitWidth = Op0->getType ()->getScalarSizeInBits ();
        if (MaskedValueIsZero (Op0, APInt::getHighBitsSet (BitWidth, BitWidth - 1), Q.DL, 0, Q.AC, Q.CxtI, Q.DT))
          {
            return Op0;
          }
        break;
    ...
    case Intrinsic::exp:
      // exp(log(x)) -> x
      if (Q.CxtI->hasAllowReassoc () && match (Op0, m_Intrinsic<Intrinsic::log> (m_Value (X))))
        {
          return X;
        }
      break;
    case Intrinsic::exp2:
      // exp2(log2(x)) -> x
      if (Q.CxtI->hasAllowReassoc () && match (Op0, m_Intrinsic<Intrinsic::log2> (m_Value (X))))
        {
          return X;
        }
      break;
    case Intrinsic::log:
      // log(exp(x)) -> x
      if (Q.CxtI->hasAllowReassoc () && match (Op0, m_Intrinsic<Intrinsic::exp> (m_Value (X))))
        {
          return X;
        }
      break;
    case Intrinsic::log2:
      ...
      if (Q.CxtI->hasAllowReassoc ()
          && (match (Op0, m_Intrinsic<Intrinsic::exp2> (m_Value (X)))
              || match (Op0, m_Intrinsic<Intrinsic::pow> (m_SpecificFP (2.0), m_Value (X)))))
        {
          return X;
        }
      break;
    case Intrinsic::log10:
      // log10(pow(10.0, x)) -> x
      if (Q.CxtI->hasAllowReassoc () && match (Op0, m_Intrinsic<Intrinsic::pow> (m_SpecificFP (10.0), m_Value (X))))
        {
          return X;
        }
      break;
    ...
    case Intrinsic::experimental_vector_reverse:
      ...
      if (match (Op0, m_Intrinsic<Intrinsic::experimental_vector_reverse> (m_Value (X))))
        {
          return X;
        }
...
/// operand that is another min/max intrinsic with shared operand(s). The
/// caller is expected to swap the operand arguments to handle commutation.
static Value *
foldMinMaxSharedOp (Intrinsic::ID IID, Value *Op0, Value *Op1)
{
  Value *X;
  Value *Y;
  ...
  if (Op1 == X || Op1 == Y || match (Op1, m_c_MaxOrMin (m_Specific (X), m_Specific (Y))))
    {
...
/// returns null.
static Value *
SimplifyFSubInst (Value *Op0, Value *Op1, FastMathFlags FMF, const SimplifyQuery &Q, unsigned MaxRecurse,
                  fp::ExceptionBehavior ExBehavior, RoundingMode Rounding)
{
  if (isDefaultFPEnvironment (ExBehavior, Rounding))
    {
  ...
  if (match (Op1, m_NegZeroFP ()) && (FMF.noSignedZeros () || CannotBeNegativeZero (Op0, Q.TLI)))
    {
      return Op0;
    }
  ...
  if (FMF.noSignedZeros () && match (Op0, m_AnyZeroFP ())
      && (match (Op1, m_FSub (m_AnyZeroFP (), m_Value (X))) || match (Op1, m_FNeg (m_Value (X)))))
    {
      return X;
    }
  ...
  // (X + Y) - Y --> X
  if (FMF.noSignedZeros () && FMF.allowReassoc ()
      && (match (Op1, m_FSub (m_Specific (Op0), m_Value (X))) || match (Op0, m_c_FAdd (m_Specific (Op1), m_Value (X)))))
    {
      return X;
    }
...
static Value *
simplifyBinaryIntrinsic (Function *F, Value *Op0, Value *Op1, const SimplifyQuery &Q)
{
  Intrinsic::ID IID        = F->getIntrinsicID ();
  ...
  switch (IID)
    ...
    case Intrinsic::abs:
      ...
      // It is always ok to pick the earlier abs. We'll just lose nsw if its
      // only on the outer abs.
    ...
    case Intrinsic::umin:
      {
        ...
        if (Q.isUndefValue (Op1))
          {
            return ConstantInt::get (ReturnType, MinMaxIntrinsic::getSaturationPoint (IID, BitWidth));
          }
        ...
        if (match (Op1, m_APIntAllowUndef (C)))
          {
            ...
            // If the constant op is the opposite of the limit value, the other
            // must be larger/smaller or equal. For example: umin(i8 %x, i8
            // 255) --> %x
            if (*C == MinMaxIntrinsic::getSaturationPoint (getInverseMinMaxIntrinsic (IID), BitWidth))
              {
                return Op0;
              }
            ...
            if ((MinMax0 != nullptr) && MinMax0->getIntrinsicID () == IID)
              {
                // TODO: loosen undef/splat restrictions for vector constants.
                Value       *M00 = MinMax0->getOperand (0);
                Value       *M01 = MinMax0->getOperand (1);
                const APInt *InnerC;
        ...
        ICmpInst::Predicate Pred = ICmpInst::getNonStrictPredicate (MinMaxIntrinsic::getPredicate (IID));
        ...
        if (Optional<bool> Imp = isImpliedByDomCondition (Pred, Op0, Op1, Q.CxtI, Q.DL))
          {
            return *Imp ? Op0 : Op1;
          }
        if (Optional<bool> Imp = isImpliedByDomCondition (Pred, Op1, Op0, Q.CxtI, Q.DL))
          {
            return *Imp ? Op1 : Op0;
          }
    ...
    case Intrinsic::sadd_with_overflow:
      ...
      if (Q.isUndefValue (Op0) || Q.isUndefValue (Op1))
        {
          return ConstantStruct::get (cast<StructType> (ReturnType),
    ...
    case Intrinsic::minimum:
      {
        ...
        const APFloat *C;
        if (match (Op1, m_APFloat (C)) && (C->isInfinity () || (Q.CxtI->hasNoInfs () && C->isLargest ())))
          {
            // minnum(X, -inf) -> -inf
            // maxnum(X, +inf) -> +inf
        ...
        if (auto *M0 = dyn_cast<IntrinsicInst> (Op0))
          {
            if (M0->getIntrinsicID () == IID && (M0->getOperand (0) == Op1 || M0->getOperand (1) == Op1))
              {
                return Op0;
              }
          }
        if (auto *M1 = dyn_cast<IntrinsicInst> (Op1))
          {
            if (M1->getIntrinsicID () == IID && (M1->getOperand (0) == Op0 || M1->getOperand (1) == Op0))
              {
                return Op1;
              }
          }
    ...
    case Intrinsic::experimental_vector_extract:
      {
        ...
        Value   *X    = nullptr;
        if (match (Op0, m_Intrinsic<Intrinsic::experimental_vector_insert> (m_Value (), m_Value (X), m_Zero ()))
...
simplifyIntrinsic (CallBase *Call, const SimplifyQuery &Q)
{
  ...
  if (NumOperands == 0u)
    {
      switch (IID)
        ...
        case Intrinsic::vscale:
          {
            // Call may not be inserted into the IR yet at point of calling
            // simplify.
  ...
  if (NumOperands == 2)
    {
      return simplifyBinaryIntrinsic (F, Call->getArgOperand (0), Call->getArgOperand (1), Q);
    }
  ...
  switch (IID)
    ...
    case Intrinsic::masked_gather:
      {
        ...
        // If the mask is all zeros or undef, the "passthru" argument is the
        // result.
    ...
    case Intrinsic::fshr:
      {
        ...
        Value *ShAmtArg = Call->getArgOperand (2);
    ...
    case Intrinsic::experimental_constrained_fma:
      {
        ...
        if (Value *V = simplifyFPOp ({Op0, Op1, Op2}, {}, Q, FPI->getExceptionBehavior ().getValue (),
    ...
    case Intrinsic::fmuladd:
      {
        ...
        Value *Op1 = Call->getArgOperand (1);
        Value *Op2 = Call->getArgOperand (2);
        if (Value *V = simplifyFPOp ({Op0, Op1, Op2}, {}, Q, fp::ebIgnore, RoundingMode::NearestTiesToEven))
          {
            return V;
          }
        return nullptr;
    ...
    case Intrinsic::smul_fix_sat:
      {
        ...
        // Canonicalize constant operand as Op1 (ConstantFolding handles the
        // case when both Op0 and Op1 are constant so we do not care about that
        // special case here).
        if (isa<Constant> (Op0))
          {
            std::swap (Op0, Op1);
        ...
        // X * (1 << Scale) -> X
        APInt ScaledOne
            = APInt::getOneBitSet (ReturnType->getScalarSizeInBits (), cast<ConstantInt> (Op2)->getZExtValue ());
    ...
    case Intrinsic::experimental_vector_insert:
      {
        ...
        Value   *X    = nullptr;
        if (match (SubVec, m_Intrinsic<Intrinsic::experimental_vector_extract> (m_Value (X), m_Zero ()))
            && (Q.isUndefValue (Vec) || Vec == X) && IdxN == 0 && X->getType () == ReturnType)
          {
            return X;
          }
    ...
    case Intrinsic::experimental_constrained_fadd:
      {
        auto *FPI = cast<ConstrainedFPIntrinsic> (Call);
        return SimplifyFAddInst (FPI->getArgOperand (0), FPI->getArgOperand (1), FPI->getFastMathFlags (), Q,
                                 FPI->getExceptionBehavior ().getValue (), FPI->getRoundingMode ().getValue ());
        break;
      }
    case Intrinsic::experimental_constrained_fsub:
      {
        auto *FPI = cast<ConstrainedFPIntrinsic> (Call);
        return SimplifyFSubInst (FPI->getArgOperand (0), FPI->getArgOperand (1), FPI->getFastMathFlags (), Q,
                                 FPI->getExceptionBehavior ().getValue (), FPI->getRoundingMode ().getValue ());
        break;
      }
    case Intrinsic::experimental_constrained_fmul:
      {
        auto *FPI = cast<ConstrainedFPIntrinsic> (Call);
        return SimplifyFMulInst (FPI->getArgOperand (0), FPI->getArgOperand (1), FPI->getFastMathFlags (), Q,
                                 FPI->getExceptionBehavior ().getValue (), FPI->getRoundingMode ().getValue ());
        break;
      }
    case Intrinsic::experimental_constrained_fdiv:
      {
        auto *FPI = cast<ConstrainedFPIntrinsic> (Call);
        return SimplifyFDivInst (FPI->getArgOperand (0), FPI->getArgOperand (1), FPI->getFastMathFlags (), Q,
                                 FPI->getExceptionBehavior ().getValue (), FPI->getRoundingMode ().getValue ());
        break;
      }
    case Intrinsic::experimental_constrained_frem:
      {
        auto *FPI = cast<ConstrainedFPIntrinsic> (Call);
        return SimplifyFRemInst (FPI->getArgOperand (0), FPI->getArgOperand (1), FPI->getFastMathFlags (), Q,
                                 FPI->getExceptionBehavior ().getValue (), FPI->getRoundingMode ().getValue ());
        break;
      }
    default:
...
static Value *
SimplifyFreezeInst (Value *Op0, const SimplifyQuery &Q)
{
...
static Value *
SimplifyLoadInst (LoadInst *LI, Value *PtrOp, const SimplifyQuery &Q)
{
  if (LI->isVolatile ())
    {
      return nullptr;
  ...
  if ((PtrOpC == nullptr) && isa<Constant> (getUnderlyingObject (PtrOp)))
    {
      PtrOp = PtrOp->stripAndAccumulateConstantOffsets (Q.DL, Offset,
...
static Value *
CocosimplifyInstructionWithOperands (Instruction *I, ArrayRef<Value *> NewOps, const SimplifyQuery &SQ,
                                     OptimizationRemarkEmitter *ORE)
{
  const SimplifyQuery Q      = (SQ.CxtI != nullptr) ? SQ : SQ.getWithInstruction (I);
  ...
  switch (I->getOpcode ())
      ...
      if (llvm::all_of (NewOps, [] (Value *V) { return isa<Constant> (V); }))
        {
          ...
          transform (NewOps, NewConstOps.begin (), [] (Value *V) { return cast<Constant> (V); });
          Result = ConstantFoldInstOperands (I, NewConstOps, Q.DL, Q.TLI);
        }
      break;
    case Instruction::FNeg:
      Result = Coco::simplifyFNegInst (NewOps[0], I->getFastMathFlags (), Q, 1000);
      break;
    case Instruction::FAdd:
      Result = Coco::SimplifyFAddInst (NewOps[0], NewOps[1], I->getFastMathFlags (), Q, 1000);
      break;
    case Instruction::Add:
      Result = Coco::SimplifyAddInst (NewOps[0], NewOps[1], Q.IIQ.hasNoSignedWrap (cast<BinaryOperator> (I)),
                                      Q.IIQ.hasNoUnsignedWrap (cast<BinaryOperator> (I)), Q, 1000);
      break;
    case Instruction::FSub:
      Result = Coco::SimplifyFSubInst (NewOps[0], NewOps[1], I->getFastMathFlags (), Q, 1000);
      break;
    case Instruction::Sub:
      Result = Coco::SimplifySubInst (NewOps[0], NewOps[1], Q.IIQ.hasNoSignedWrap (cast<BinaryOperator> (I)),
                                      Q.IIQ.hasNoUnsignedWrap (cast<BinaryOperator> (I)), Q, 1000);
      break;
    case Instruction::FMul:
      Result = Coco::SimplifyFMulInst (NewOps[0], NewOps[1], I->getFastMathFlags (), Q, 1000);
      break;
    case Instruction::Mul:
      Result = Coco::SimplifyMulInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::SDiv:
      Result = Coco::SimplifySDivInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::UDiv:
      Result = Coco::SimplifyUDivInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::FDiv:
      Result = Coco::SimplifyFDivInst (NewOps[0], NewOps[1], I->getFastMathFlags (), Q, 1000);
      break;
    case Instruction::SRem:
      Result = Coco::SimplifySRemInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::URem:
      Result = Coco::SimplifyURemInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::FRem:
      Result = Coco::SimplifyFRemInst (NewOps[0], NewOps[1], I->getFastMathFlags (), Q, 1000);
      break;
    case Instruction::Shl:
      Result = Coco::SimplifyShlInst (NewOps[0], NewOps[1], Q.IIQ.hasNoSignedWrap (cast<BinaryOperator> (I)),
                                      Q.IIQ.hasNoUnsignedWrap (cast<BinaryOperator> (I)), Q, 1000);
      break;
    case Instruction::LShr:
      Result = Coco::SimplifyLShrInst (NewOps[0], NewOps[1], Q.IIQ.isExact (cast<BinaryOperator> (I)), Q, 1000);
      break;
    case Instruction::AShr:
      Result = Coco::SimplifyAShrInst (NewOps[0], NewOps[1], Q.IIQ.isExact (cast<BinaryOperator> (I)), Q, 1000);
      break;
    case Instruction::And:
      Result = Coco::SimplifyAndInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::Or:
      Result = Coco::SimplifyOrInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::Xor:
      Result = Coco::SimplifyXorInst (NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::ICmp:
      Result = Coco::SimplifyICmpInst (cast<ICmpInst> (I)->getPredicate (), NewOps[0], NewOps[1], Q, 1000);
      break;
    case Instruction::FCmp:
      Result = Coco::SimplifyFCmpInst (cast<FCmpInst> (I)->getPredicate (), NewOps[0], NewOps[1],
                                       I->getFastMathFlags (), Q, 1000);
      break;
    case Instruction::Select:
      Result = Coco::SimplifySelectInst (NewOps[0], NewOps[1], NewOps[2], Q, 1000);
      break;
    case Instruction::GetElementPtr:
      {
        auto *GEPI = cast<GetElementPtrInst> (I);
        Result     = Coco::SimplifyGEPInst (GEPI->getSourceElementType (), NewOps[0], makeArrayRef (NewOps).slice (1),
                                            GEPI->isInBounds (), Q, 1000);
        break;
      }
    case Instruction::InsertValue:
      {
        InsertValueInst *IV = cast<InsertValueInst> (I);
        Result              = Coco::SimplifyInsertValueInst (NewOps[0], NewOps[1], IV->getIndices (), Q, 1000);
        break;
      }
    case Instruction::InsertElement:
      {
        Result = Coco::SimplifyInsertElementInst (NewOps[0], NewOps[1], NewOps[2], Q);
        break;
      }
    case Instruction::ExtractValue:
      {
        auto *EVI = cast<ExtractValueInst> (I);
        Result    = Coco::SimplifyExtractValueInst (NewOps[0], EVI->getIndices (), Q, 1000);
        break;
      }
    case Instruction::ExtractElement:
      {
        Result = Coco::SimplifyExtractElementInst (NewOps[0], NewOps[1], Q, 1000);
        break;
      }
    case Instruction::ShuffleVector:
      {
        ...
        Result
            = Coco::SimplifyShuffleVectorInst (NewOps[0], NewOps[1], SVI->getShuffleMask (), SVI->getType (), Q, 1000);
        break;
      }
    case Instruction::PHI:
      Result = Coco::SimplifyPHINode (cast<PHINode> (I), NewOps, Q);
      break;
    ...
    case Instruction::Freeze:
      Result = Coco::SimplifyFreezeInst (NewOps[0], Q);
      break;
#define HANDLE_CAST_INST(num, opc, clas) case Instruction::opc:
#include "llvm/IR/Instruction.def"
#undef HANDLE_CAST_INST
      Result = Coco::SimplifyCastInst (I->getOpcode (), NewOps[0], I->getType (), Q);
      break;
    ...
    case Instruction::Load:
      Result = Coco::SimplifyLoadInst (cast<LoadInst> (I), NewOps[0], Q);
      break;
    }
...
CocoSimplifyInstructionWithOperands (Instruction *I, ArrayRef<Value *> NewOps, const SimplifyQuery &SQ,
                                     OptimizationRemarkEmitter *ORE)
{
  assert (NewOps.size () == I->getNumOperands () && "Number of operands should match the instruction!");
  return CocosimplifyInstructionWithOperands (I, NewOps, SQ, ORE);
}

static Value *
CocoSimplifyInstruction (Instruction *I, const SimplifyQuery &SQ, OptimizationRemarkEmitter *ORE)
{
...
static bool
runImpl (Function &F, const SimplifyQuery &SQ, OptimizationRemarkEmitter *ORE)
{
  ...
  do
    {
      for (BasicBlock &BB : F)
        {
          // Unreachable code can take on strange forms that we are not
          // prepared to handle. For example, an instruction may have itself as
          // an operand.
          ...
          for (Instruction &I : BB)
            {
              // The first time through the loop, ToSimplify is empty and we
              // try to simplify all instructions. On later iterations,
              // ToSimplify is not empty and we only bother simplifying
              // instructions that are in it.
              if (!ToSimplify->empty () && (ToSimplify->count (&I) == 0u))
                {
                  continue;
                }
              ...
              if (isInstructionTriviallyDead (&I))
                ...
                {
                  if (Value *V = Coco::CocoSimplifyInstruction (&I, SQ, ORE))
                    {
                      ...
                      ++NumSimplified;
                      Changed = true;
                      // A call can get simplified, but it may not be trivially
                      // dead.
                      if (isInstructionTriviallyDead (&I))
                        {
                          DeadInstsInBB.push_back (&I);
...
{
  return {LLVM_PLUGIN_API_VERSION, "InstSimplify", LLVM_VERSION_STRING, [] (PassBuilder &PB) {
